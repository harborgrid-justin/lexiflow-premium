...
export class AuthController {
  ...
  async login(@Body() loginDto: LoginDto) {
    console.log("Login DTO received:", JSON.stringify(loginDto));
  ...
  @ApiResponse({ status: 409, description: "Resource already exists" })
  async logout(
    @Request() req: RequestWithTokenPayload,
  ...
  async verifyMfa(@Body() verifyMfaDto: VerifyMfaDto) {
    return this.authService.verifyMfa(verifyMfaDto.token, verifyMfaDto.code);
  }
