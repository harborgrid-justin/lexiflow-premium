...
import {
  TimeEntry,
  TimeEntryStatus,
} from "./time-entries/entities/time-entry.entity";
...
export class BillingService {
  ...
  async getWIPStats(): Promise<Array<Record<string, unknown>>> {
    ...
    const unbilledExpenses = await this.expenseRepository.find({
      where: {
        status: Not(ExpenseStatus.BILLED),
      } as unknown as FindOptionsWhere<Expense>,
    });
  ...
  async getOverviewStats(): Promise<{
    ...
    try {
      ...
      const totalTime = allTimeEntries.reduce(
        (sum, e) => sum + (e.total || 0),
        0
      );
