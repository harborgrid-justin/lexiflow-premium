...
/**
 * Extended Socket interface with user ID
 */
  ...
  implements OnGatewayConnection, OnGatewayDisconnect, OnModuleDestroy
{
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      if (!userId) {
        this.logger.warn(
          `Client ${client.id} authentication failed - no valid user ID`
        );
        client.emit("error", {
          code: "AUTH_FAILED",
      ...
      const existingConnections =
        this.userSockets.get(userId) || new Set<string>();
  ...
  handleDisconnect(client: Socket) {
    ...
    if (userId) {
      ...
      this.logger.log(
        `Client disconnected: ${client.id} (no user mapping found)`
      );
    }
  }
  ...
  handleMessageSend(
    ...
    if (messageSize > this.MAX_MESSAGE_SIZE) {
      this.logger.warn(
        `Message from ${userId} exceeds size limit: ${messageSize} bytes`
      );
      client.emit("error", {
    ...
    this.logger.log(
      `Message sent by ${userId} to conversation ${data.conversationId}`
    );
  ...
  handleJoinConversation(
    ...
    if (!limitCheck.allowed) {
      this.logger.warn(
        `Client ${client.id} cannot join ${roomId}: ${limitCheck.reason}`
      );
      client.emit("error", {
  ...
  private async extractUserIdFromSocket(
    ...
    try {
      ...
      if (!token) {
        this.logger.warn(
          "No authentication token provided in WebSocket handshake"
        );
        return null;
      }
      ...
      if (!jwtSecret) {
        this.logger.error(
          "JWT secret not configured - authentication disabled"
        );
        return null;
      }
      ...
      this.logger.error(
        "JWT verification failed for WebSocket connection:",
        message
      );
      return null;
    }
  ...
  sendToUser(userId: string, event: string, data: unknown) {
    ...
    if (socketIds && socketIds.size > 0) {
      ...
      this.logger.debug(
        `Event ${event} sent to user ${userId} (${socketIds.size} connections)`
      );
    } else {
      this.logger.debug(
        `User ${userId} has no active connections - event ${event} not sent`
      );
    }
  }
  ...
  getStats() {
    ...
    return {
      ...
      userDetails: Array.from(this.userSockets.entries()).map(
        ([userId, sockets]) => ({
          userId,
