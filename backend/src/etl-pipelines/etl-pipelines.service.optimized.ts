...
export class EtlPipelinesService implements OnModuleDestroy {
  ...
  private pipelineCache: Map<string, { config: unknown; timestamp: number }> =
    new Map();
  private transformationCache: Map<
    string,
    { fn: (...args: unknown[]) => unknown; timestamp: number }
  > = new Map();
  ...
  async executePipeline<T>(config: PipelineConfig<T>): Promise<PipelineResult> {
    ...
    try {
      ...
      await this.processStreamWithBackpressure(
        stream,
        config as PipelineConfig<unknown>,
        execution
      );
      ...
      const result: PipelineResult = {
        ...
        throughput:
          execution.processed / ((Date.now() - execution.startTime) / 1000),
      };
      ...
      this.logger.error(
        `Pipeline ${pipelineId} failed: ${message}`,
   ...
   */
  private async createSourceStream<T>(
    _source: DataSource | unknown
