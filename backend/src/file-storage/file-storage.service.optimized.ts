...
export class FileStorageService implements OnModuleDestroy {
  ...
  // Caches
  private metadataCache: Map<string, { data: unknown; timestamp: number }> =
    new Map();
  private versionCache: Map<string, { data: unknown; timestamp: number }> =
    new Map();
  private checksumCache: Map<string, { data: string; timestamp: number }> =
    new Map();
  ...
  private initializeBufferPool(): void {
    ...
    this.logger.log(
      `Initialized buffer pool with ${this.MAX_BUFFER_POOL_SIZE} buffers`
    );
  }
  ...
  async *downloadFile(fileId: string): AsyncGenerator<Buffer> {
    ...
    try {
      const metadata = (await this.getFileMetadata(fileId)) as { size: number };
      ...
      if (shouldStream) {
        ...
        for (let i = 0; i < chunkCount; i++) {
          ...
          const buffer =
            this.bufferPool.pop() || Buffer.allocUnsafe(this.MAX_CHUNK_SIZE);
  ...
  async getFileVersions(fileId: string): Promise<FileVersion[]> {
    ...
    if (cached && Date.now() - cached.timestamp < this.CACHE_TTL_MS) {
      return cached.data as FileVersion[];
    }
    ...
    const versions = Array.from({ length: versionCount }, (_, i) => ({
      ...
      checksum: this.generateMockChecksum(),
      createdAt: new Date(
        Date.now() - (versionCount - i) * 24 * 60 * 60 * 1000
      ),
      createdBy: "user123",
  ...
  async calculateChecksum(fileId: string): Promise<string> {
    ...
    if (cached && Date.now() - cached.timestamp < this.CACHE_TTL_MS * 4) {
      // 1-hour cache
      return cached.data;
    }
  ...
  async batchGetMetadata(
    ...
    const batchResults = await Promise.all(
      fileIds.map(async (fileId) => {
        try {
          ...
          return [fileId, metadata] as [string, FileMetadata];
        } catch (error) {
          this.logger.error(`Failed to get metadata for ${fileId}:`, error);
          return [fileId, null] as [string, FileMetadata | null];
        }
      })
    );
    ...
    this.logger.log(
      `Completed batch metadata retrieval for ${results.size}/${fileIds.length} files`
    );
    return results;
  }
  ...
  getMemoryStats(): MemoryStats {
    return {
      ...
      memoryUsage: {
        ...
        bufferPoolMB: (
          (this.bufferPool.length * this.MAX_CHUNK_SIZE) /
          1024 /
          1024
        ).toFixed(2),
      },
