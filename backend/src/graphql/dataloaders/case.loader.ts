...
export class CaseLoader {
   ...
   */
  public readonly batchCases = new DataLoader(
   ...
   */
  public readonly batchPartiesByCaseId = new DataLoader(
    async (caseIds: readonly string[]) => {
      const cases = await this.caseRepository.find({
        where: { id: In([...caseIds]) },
        relations: ["parties"],
      ...
      return caseIds.map((id) => (partiesByCaseId.get(id) as unknown[]) || []);
    }
  );
   ...
   */
  public readonly batchPhasesByCaseId = new DataLoader(
    async (caseIds: readonly string[]) => {
      // Note: Case entity doesn't have a 'phases' relation - this needs to be implemented
      // or removed based on your schema design
      const phasesByCaseId = new Map<string, unknown[]>();
      caseIds.forEach((id) => phasesByCaseId.set(id, []));
      return caseIds.map((id) => (phasesByCaseId.get(id) as unknown[]) || []);
    }
  );
   ...
   */
  public readonly batchMotionsByCaseId = new DataLoader(
    async (caseIds: readonly string[]) => {
   ...
   */
  public readonly batchDocketEntriesByCaseId = new DataLoader(
    async (caseIds: readonly string[]) => {
