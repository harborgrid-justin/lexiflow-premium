...
export class DocumentLoader {
   ...
   */
  public readonly batchDocuments = new DataLoader(
   ...
   */
  public readonly batchDocumentsByCaseId = new DataLoader(
    async (caseIds: readonly string[]) => {
      const documents = await this.documentRepository.find({
        where: { caseId: In([...caseIds]) },
      });
      const documentsByCaseId = new Map<string, Document[]>();
      ...
      return caseIds.map(
        (id) => (documentsByCaseId.get(id) as Document[]) || []
      );
    }
  );
   ...
   */
  public readonly batchVersionsByDocumentId = new DataLoader(
    async (documentIds: readonly string[]) => {
      ...
      return documentIds.map(
        (id) => (versionsByDocId.get(id) as DocumentVersion[]) || []
      );
    }
  );
   ...
   */
  public readonly batchClausesByDocumentId = new DataLoader(
    async (documentIds: readonly string[]) => {
   ...
   */
  public readonly batchCreatedByForDocuments = new DataLoader(
    async (documentIds: readonly string[]) => {
      const documents = await this.documentRepository.find({
        where: { id: In([...documentIds]) },
        select: ["id", "createdBy"],
