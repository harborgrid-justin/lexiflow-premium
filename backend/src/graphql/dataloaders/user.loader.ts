...
export class UserLoader {
   ...
   */
  public readonly batchUsersByEmail = new DataLoader(
   ...
   */
  public readonly batchPermissionsByUserId = new DataLoader(
    async (userIds: readonly string[]) => {
      const users = await this.userRepository.find({
        where: { id: In([...userIds]) },
      });
      ...
      return userIds.map(
        (id) => (permissionsByUserId.get(id) as string[]) || []
      );
    }
  );
   ...
   */
  public readonly batchPreferencesByUserId = new DataLoader(
    async (userIds: readonly string[]) => {
