...
export class UserResolver {
  ...
  async getUsers(
    ...
    if (filter?.role && filter.role.length > 0) {
      return users.filter((u) =>
        filter.role?.includes(u.role)
      ) as unknown as UserType[];
    }
    if (filter?.isActive !== undefined) {
      return users.filter(
        (u) => u.isActive === filter.isActive
      ) as unknown as UserType[];
    }
  ...
  async enableMfa(@CurrentUser() user: AuthenticatedUser): Promise<boolean> {
    ...
    throw new Error(
      "MFA setup requires QR code scanning - use REST API endpoint"
    );
  }
  ...
  async disableMfa(): Promise<boolean> {
    ...
    throw new Error(
      "MFA disable requires verification code - use REST API endpoint"
    );
  }
