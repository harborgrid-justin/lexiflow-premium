...
const createAdapter = (): AdapterConstructor => {
  throw new Error(
    "@socket.io/redis-adapter not installed. Install with: npm install @socket.io/redis-adapter"
  );
};
...
export class RedisIoAdapter extends IoAdapter {
  ...
  async connectToRedis(): Promise<void> {
    try {
      ...
      // Create adapter (returns adapter instance directly in newer versions)
      this.adapterConstructor = createAdapter() as ReturnType<
        typeof createAdapter
      >;

      this.logger.log(
        "✅ Redis adapter for Socket.IO successfully initialized"
      );
    } catch (error) {
