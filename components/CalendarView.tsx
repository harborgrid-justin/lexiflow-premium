...
export const CalendarView: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<CalendarViewType>('master');
  ...
  const handleParentTabChange = useCallback((parentId: string) => {
    ...
    if (parent && parent.subTabs.length > 0) {
      setActiveTab(parent.subTabs[0].id as CalendarViewType);
    }
  }, []);
        ...
        {activeParentTab.subTabs.length > 1 && (
            <div className={cn("flex space-x-2 overflow-x-auto no-scrollbar py-3 px-4 md:px-6 rounded-lg border mb-4", theme.surfaceHighlight, theme.border.default)}>
                {activeParentTab.subTabs.map(tab => (
                    <button 
                        key={tab.id} 
                        onClick={() => setActiveTab(tab.id as CalendarViewType)} 
                        className={cn(
