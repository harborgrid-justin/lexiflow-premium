...
export const CalendarSync: React.FC = () => {
  ...
  const calendarIntegrations = integrations.filter((i: any) => i.type.includes('Calendar') || i.type.includes('Email'));
    ...
    <div className={cn("rounded-lg border overflow-hidden shadow-sm animate-fade-in", theme.surface, theme.border.default)}>
      ...
      <div className={cn("divide-y", theme.border.light)}>
        {calendarIntegrations.map((acc, i) => (
          <div key={i} className={cn("p-4 md:p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-0 transition-colors", `hover:${theme.surfaceHighlight}`)}>
      ...
      <div className={cn("p-6 border-t", theme.surfaceHighlight, theme.border.default)}>
