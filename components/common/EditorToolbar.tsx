  ...
  return (
    <div className={cn("flex items-center gap-1 p-2 border-b flex-wrap", theme.surfaceHighlight, theme.border.default)}>
      <div className={cn("flex items-center gap-0.5 border-r pr-2 mr-2", theme.border.default)}>
        <select className={cn("bg-transparent text-sm font-medium h-8 border-none outline-none cursor-pointer rounded px-1 transition-colors", theme.text.primary, `hover:${theme.surface}`)} onChange={(e) => onCmd('formatBlock', e.target.value)}>
