                ...
                {individuals.map((ind, idx) => (
                    <div key={idx} className={cn("p-4 border rounded-lg space-y-3 relative", theme.border.default)}>
                        <div className="grid grid-cols-2 gap-4">
                            <Input label="Name" value={ind.name} onChange={e => updateItem(setIndividuals, individuals, idx, 'name', e.target.value)} placeholder="e.g. John Smith" />
                            <Input label="Contact Info" value={ind.contact} onChange={e => updateItem(setIndividuals, individuals, idx, 'contact', e.target.value)} placeholder="Phone or Address" />
                        </div>
                        <Input label="Subject of Information" value={ind.subject} onChange={e => updateItem(setIndividuals, individuals, idx, 'subject', e.target.value)} placeholder="e.g. Witnessed the accident" />
                    </div>
                ))}
             ...
             <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                ...
                {documents.map((doc, idx) => (
                    <div key={idx} className={cn("p-4 border rounded-lg space-y-3", theme.border.default)}>
                        <Input label="Description / Category" value={doc.description} onChange={e => updateItem(setDocuments, documents, idx, 'description', e.target.value)} placeholder="e.g. Emails regarding contract negotiation" />
                        <Input label="Location" value={doc.location} onChange={e => updateItem(setDocuments, documents, idx, 'location', e.target.value)} placeholder="e.g. Corporate Exchange Server" />
                    </div>
                ))}
          ...
          {wizard.currentStep === 3 && (
             <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                ...
                {damages.map((dmg, idx) => (
                    <div key={idx} className={cn("p-4 border rounded-lg space-y-3", theme.border.default)}>
                        <div className="grid grid-cols-2 gap-4">
                             <Input label="Category" value={dmg.category} onChange={e => updateItem(setDamages, damages, idx, 'category', e.target.value)} placeholder="e.g. Lost Profits" />
                             <Input label="Claimed Amount" value={dmg.amount} onChange={e => updateItem(setDamages, damages, idx, 'amount', e.target.value)} placeholder="$0.00" />
                        </div>
                        <TextArea label="Basis of Computation" value={dmg.computation} onChange={e => updateItem(setDamages, damages, idx, 'computation', e.target.value)} rows={2} />
                    </div>
                ))}
          ...
          {wizard.currentStep === 4 && (
             <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                ...
                {insurance.map((ins, idx) => (
                    <div key={idx} className={cn("p-4 border rounded-lg space-y-3", theme.border.default)}>
                         <Input label="Insurance Carrier" value={ins.carrier} onChange={e => updateItem(setInsurance, insurance, idx, 'carrier', e.target.value)} />
                         <div className="grid grid-cols-2 gap-4">
                             <Input label="Policy Number" value={ins.policyNum} onChange={e => updateItem(setInsurance, insurance, idx, 'policyNum', e.target.value)} />
                             <Input label="Coverage Limit" value={ins.coverage} onChange={e => updateItem(setInsurance, insurance, idx, 'coverage', e.target.value)} />
                        </div>
                    </div>
                ))}
