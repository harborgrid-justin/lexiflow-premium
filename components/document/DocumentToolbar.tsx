...
export const DocumentToolbar: React.FC<DocumentToolbarProps> = ({
  ...
  return (
    <div className={cn("h-14 px-4 border-b flex justify-between items-center shrink-0", theme.surface, theme.border.default)}>
            ...
            {selectedDocsCount > 0 ? (
                <div className={cn("flex items-center px-3 py-1.5 rounded-md border animate-fade-in w-full", theme.primary.light, theme.primary.border)}>
                    ...
                    <div className="flex items-center gap-2">
                        <button className={cn("p-1 rounded", theme.primary.text, `hover:${theme.surface}`)} title="Compare" onClick={handleCompare}><Split className="h-4 w-4"/></button>
                        <button className={cn("p-1 rounded", theme.primary.text, `hover:${theme.surface}`)} title="Summarize" onClick={onBulkSummarize}>
                            {isProcessingAI ? <span className="animate-spin">...</span> : <Wand2 className="h-4 w-4"/>}
                        </button>
                        <button className={cn("p-1 rounded", theme.primary.text, `hover:${theme.surface}`)} title="Tag"><MoreHorizontal className="h-4 w-4"/></button>
                    </div>
            ...
            <div className={cn("flex p-0.5 rounded-lg border", theme.surfaceHighlight, theme.border.default)}>
                <button 
                    ...
                    className={cn(
                        ...
                        viewMode === 'list' 
                            ? cn(theme.surface, "shadow", theme.primary.text) 
