...
export const EntityAnalytics: React.FC<EntityAnalyticsProps> = ({ entities }) => {
  ...
  const { typeStats, riskStats, jurisdictionCount, topHighRisk } = useMemo(() => {
      ...
      const rStats = entities.reduce((acc, e) => {
      ...
      const highRiskEntities = entities.sort((a, b) => b.riskScore - a.riskScore).slice(0, 10).map(e => ({ name: e.name.substring(0, 10), score: e.riskScore }));
