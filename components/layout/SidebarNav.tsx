            ...
            <div className="space-y-0.5">
              {items.map(item => {
                ...
                return (
                  ...
                  >
                    <Icon className={cn("h-4 w-4 shrink-0 transition-colors", isActive ? theme.primary.text : "opacity-70 group-hover:opacity-100")} />
                    <span className="truncate">{item.label}</span>
                    {isActive && <div className={cn("absolute left-0 top-1/2 -translate-y-1/2 w-1 h-5 rounded-r-full", theme.primary.DEFAULT)}></div>}
                  </button>
                );
              })}
            </div>
