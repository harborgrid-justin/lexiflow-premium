...
import { PleadingDocument, FormattingRule, Case, PleadingSection, CaseId, UserId } from '../../types';
import { ArrowLeft, Save, Download, Eye, PenTool, GitMerge, Loader2 } from 'lucide-react';
...
// Lazy load new designer components
const PleadingPaper = lazy(() => import('./designer/PleadingPaper').then(m => ({ default: m.PleadingPaper })));
const PleadingCanvas = lazy(() => import('./designer/PleadingCanvas').then(m => ({ default: m.PleadingCanvas })));
const AIDraftingAssistant = lazy(() => import('./designer/AIDraftingAssistant').then(m => ({ default: m.AIDraftingAssistant })));
const ContextPanel = lazy(() => import('./designer/ContextPanel').then(m => ({ default: m.ContextPanel })));
const ComplianceHUD = lazy(() => import('./designer/tools/ComplianceHUD').then(m => ({ default: m.ComplianceHUD })));
const LogicOverlay = lazy(() => import('./designer/visual/LogicOverlay').then(m => ({ default: m.LogicOverlay })));
...
export const PleadingDesigner: React.FC<PleadingDesignerProps> = ({ pleading: initialDoc, onBack }) => {
  ...
  const handleInsertText = (text: string) => {
      // Logic to insert AI generated text into selected or new section
      // Placeholder: for now just appends to description of selected section
