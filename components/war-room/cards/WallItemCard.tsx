...
export const WallItemCard: React.FC<WallItemCardProps> = ({ item, onView, theme }) => {
    ...
    return (
        <div className={cn("group relative flex flex-col rounded-xl border shadow-sm transition-all hover:shadow-md cursor-pointer", theme.surface, theme.border.default, item.hot ? "ring-2 ring-red-500/20" : "")}>
            <div className={cn("aspect-[4/3] rounded-t-xl flex items-center justify-center relative overflow-hidden", theme.surfaceHighlight)}>
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); onView(); }} className={cn("p-2 rounded-full shadow-lg", theme.surface, theme.text.primary, `hover:${theme.primary.text}`)} title="View"><Eye className="h-4 w-4"/></button>
