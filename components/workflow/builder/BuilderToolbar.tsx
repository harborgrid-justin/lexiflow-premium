...
export const BuilderToolbar: React.FC<BuilderToolbarProps> = ({
  ...
  return (
    <div className={cn("h-12 border-b px-4 flex justify-between items-center z-20 shrink-0 shadow-sm", theme.surface, theme.border.default)}>
      ...
      <div className="flex items-center gap-2">
        <div className={cn("flex rounded-lg p-1 mr-2 border", theme.surfaceHighlight, theme.border.default)}>
          <button onClick={() => setScale(s => Math.max(0.5, s - 0.1))} className={cn("p-1.5 rounded", theme.text.secondary, `hover:${theme.surface}`)}><ZoomOut className="h-4 w-4"/></button>
