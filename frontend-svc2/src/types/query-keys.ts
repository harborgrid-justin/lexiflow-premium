 ...
 *
 * NOTE: The runtime values for these keys are defined in @/utils/queryKeys.ts.
 * Please ensure types here are consistent with the values there.
 *
...
export type GenericEntityQueryKeys<TEntity extends string> = {
  ...
  ) => readonly [TEntity, "filter", Record<string, unknown>];
};
...
export type BillingQueryKeys = {
  timeEntries: {
    ...
    byCase: (
      caseId: string
    ) => readonly ["billing", "timeEntries", "case", string];
    byUser: (
      userId: string
    ) => readonly ["billing", "timeEntries", "user", string];
    byStatus: (
      status: string
    ) => readonly ["billing", "timeEntries", "status", string];
  };
  invoices: {
    ...
    byCase: (
      caseId: string
    ) => readonly ["billing", "invoices", "case", string];
    byClient: (
      clientId: string
    ) => readonly ["billing", "invoices", "client", string];
    byStatus: (
      status: string
    ) => readonly ["billing", "invoices", "status", string];
  };
  trustAccounts: {
    ...
    transactions: (
      accountId: string
    ) => readonly ["billing", "trustAccounts", string, "transactions"];
  };
  stats: {
    wip: (caseId?: string) => readonly ["billing", "stats", "wip", string?];
    realization: (
      period?: string
    ) => readonly ["billing", "stats", "realization", string?];
    overview: () => readonly ["billing", "stats", "overview"];
    performance: (
      period: string
    ) => readonly ["billing", "stats", "performance", string];
  };
  rates: {
    all: () => readonly ["billing", "rates"];
    byTimekeeper: (
      timekeeperId: string
    ) => readonly ["billing", "rates", "timekeeper", string];
  };
};
...
export type WorkflowQueryKeys = {
  processes: {
    ...
    byCase: (
      caseId: string
    ) => readonly ["workflow", "processes", "case", string];
    byStatus: (
      status: string
    ) => readonly ["workflow", "processes", "status", string];
  };
  templates: {
    ...
    byCategory: (
      category: string
    ) => readonly ["workflow", "templates", "category", string];
  };
  tasks: {
    all: () => readonly ["workflow", "tasks"];
    byProcess: (
      processId: string
    ) => readonly ["workflow", "tasks", "process", string];
    byAssignee: (
      userId: string
    ) => readonly ["workflow", "tasks", "assignee", string];
  };
  analytics: {
    all: () => readonly ["workflow", "analytics"];
    byPeriod: (
      period: string
    ) => readonly ["workflow", "analytics", "period", string];
  };
};
...
export type TrialQueryKeys = {
  exhibits: {
    ...
    byStatus: (
      status: string
    ) => readonly ["trial", "exhibits", "status", string];
  };
  jurors: {
    ...
    byStatus: (
      status: string
    ) => readonly ["trial", "jurors", "status", string];
  };
  witnesses: {
...
export type OrganizationQueryKeys = {
  ...
  byJurisdiction: (
    jurisdiction: string
  ) => readonly ["organizations", "jurisdiction", string];
...
export type UserQueryKeys = {
  ...
  byDepartment: (
    department: string
  ) => readonly ["users", "department", string];
...
export type RiskQueryKeys = {
  ...
  byProbability: (
    probability: string
  ) => readonly ["risks", "probability", string];
...
export type RuleQueryKeys = {
  ...
  byJurisdiction: (
    jurisdiction: string
  ) => readonly ["rules", "jurisdiction", string];
...
export type AnalysisQueryKeys = {
  sessions: {
    ...
    byCase: (
      caseId: string
    ) => readonly ["analysis", "sessions", "case", string];
  };
  judgeProfiles: {
    all: () => readonly ["analysis", "judgeProfiles"];
    byJudge: (
      judgeName: string
    ) => readonly ["analysis", "judgeProfiles", string];
  };
  counselProfiles: {
    all: () => readonly ["analysis", "counselProfiles"];
    byCounsel: (
      counselName: string
    ) => readonly ["analysis", "counselProfiles", string];
  };
  predictions: {
    byCase: (
      caseId: string
    ) => readonly ["analysis", "predictions", "case", string];
  };
};
