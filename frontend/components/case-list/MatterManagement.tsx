        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
              Centralized case oversight, intake pipeline, and resource coordination
            </p>
          </div>
          <button
          ...
          <div className="grid grid-cols-4 gap-4 mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
            <div>
              ...
              <select
                id="status-filter"
                value={statusFilter}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setStatusFilter(e.target.value as MatterStatus | 'all')}
            ...
            <div>
              ...
              <select
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPriorityFilter(e.target.value as MatterPriority | 'all')}
                className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-900 dark:text-slate-100"
              >
            ...
            <div>
              ...
              <select
                id="type-filter"
                value={typeFilter}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTypeFilter(e.target.value as MatterType | 'all')}
                className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-900 dark:text-slate-100"
              >
            ...
            <div>
              <label htmlFor="practice-area-filter" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
                Practice Area
              </label>
              <select
                id="practice-area-filter"
                value={practiceAreaFilter}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPracticeAreaFilter(e.target.value as PracticeArea | 'all')}
                className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-900 dark:text-slate-100"
              >
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(matter.status)}`}>
                        {getStatusIcon(matter.status)}
                        {matter.status.replace(/_/g, ' ')}
                      </span>
                      <span className={`text-xs font-medium ${getPriorityColor(matter.priority)}`}>
                        {matter.priority}
                      </span>
                    </div>
