  ...
  const openScheduleWindow = () => {
      openWindow(
          ...
          <div className="p-6 space-y-4 bg-white">
                <Input label="Witness Name" value={newDepo.witnessName || ''} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setNewDepo({...newDepo, witnessName: e.target.value})} placeholder="e.g. Jane Doe"/>
                <div className="grid grid-cols-2 gap-4">
                    <Input label="Date" type="date" value={newDepo.date || ''} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setNewDepo({...newDepo, date: e.target.value})}/>
                    <Input label="Location" value={newDepo.location || ''} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setNewDepo({...newDepo, location: e.target.value})} placeholder="e.g. Remote (Zoom)"/>
                </div>
                <Input label="Court Reporter / Vendor" value={newDepo.courtReporter || ''} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setNewDepo({...newDepo, courtReporter: e.target.value})} placeholder="e.g. Veritext"/>
