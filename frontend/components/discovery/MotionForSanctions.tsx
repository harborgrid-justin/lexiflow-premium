        ...
        <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen()} title="Draft Sanctions Motion">
            <div className="p-6 space-y-4">
                ...
                <div>
                    ...
                    <select className={cn("w-full p-2 border rounded text-sm", theme.surface.default, theme.border.default, theme.text.primary)} value={newMotion.ruleBasis} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewMotion({...newMotion, ruleBasis: e.target.value as any})}>
                ...
                <TextArea label="Argument Summary" value={newMotion.description || ''} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewMotion({...newMotion, description: e.target.value})} rows={4} placeholder="Describe the violation..."/>
