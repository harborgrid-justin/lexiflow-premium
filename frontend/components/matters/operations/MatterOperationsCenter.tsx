...
import {
  ...
  Activity, Filter, Search, Plus, List, Grid, Kanban
} from 'lucide-react';
    ...
    <div className={cn('h-full flex flex-col', theme === 'dark' ? 'bg-slate-900' : 'bg-slate-50')}>
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className={cn('text-sm mt-1', theme === 'dark' ? 'text-slate-400' : 'text-slate-600')}>
              Task management, workflow, and team collaboration
            </p>
          </div>
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <StatCard title="Active Tasks" value="47" theme={theme} />
          <StatCard title="Due Today" value="8" theme={theme} />
          <StatCard title="In Progress" value="23" theme={theme} />
          <StatCard title="Completed" value="156" theme={theme} />
        </div>
...
const TaskItem: React.FC<{ theme: string }> = ({ theme }) => (
...
const ActivityItem: React.FC<{ theme: string }> = ({ theme }) => (
