...
export const ANALYTICS_QUERY_KEYS = {
    ...
    judgeMotions: (judgeName: string) => ['analytics', 'judge-motions', judgeName] as const,
    outcomePredictions: () => ['analytics', 'outcome-predictions'] as const,
    outcomePrediction: (caseId: string) => ['analytics', 'outcome-prediction', caseId] as const,
} as const;
...
export const AnalyticsService = {
    ...
    getJudgeMotionStats: async (): Promise<JudgeMotionStat[]> => {
        try {
            ...
            if (!validStats) {
                console.warn('[AnalyticsService] Invalid judge stats structure, using default data');
                return DEFAULT_JUDGE_MOTION_STATS;
            }
