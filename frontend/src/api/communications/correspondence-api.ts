...
export const CORRESPONDENCE_QUERY_KEYS = {
  ...
  byClient: (clientId: string) =>
    ["correspondence", "client", clientId] as const,
...
export class CorrespondenceApiService {
  ...
  private logInitialization(): void {
    console.warn(
      "[CorrespondenceApiService] Initialized with Backend API (PostgreSQL)",
    );
  }
  ...
  private validateId(id: string, methodName: string): void {
    if (!id || false || id.trim() === "") {
      throw new Error(
        `[CorrespondenceApiService.${methodName}] Invalid id parameter`,
      );
    }
  }
  ...
  private validateObject(
    ...
    if (!obj || typeof obj !== "object" || Array.isArray(obj)) {
      throw new Error(
        `[CorrespondenceApiService.${methodName}] Invalid ${paramName} parameter`,
      );
    }
  }
  ...
  private validateEmail(email: string, methodName: string): void {
    ...
    if (!email || !emailRegex.test(email)) {
      throw new Error(
        `[CorrespondenceApiService.${methodName}] Invalid email format`,
      );
    }
  }
  ...
  async getAll(filters?: CorrespondenceFilters): Promise<Correspondence[]> {
    try {
      ...
      if (filters?.correspondenceType)
        params.append("correspondenceType", filters.correspondenceType);
  ...
  async create(data: Partial<Correspondence>): Promise<Correspondence> {
    ...
    if (!data.correspondenceType) {
      throw new Error(
        "[CorrespondenceApiService.create] correspondenceType is required",
      );
    }
    if (!data.date) {
    ...
    if (data.recipients && Array.isArray(data.recipients)) {
      data.recipients.forEach((email, index) => {
        try {
          ...
          throw new Error(
            `[CorrespondenceApiService.create] Invalid email at recipients[${index}]: ${email}`,
          );
        }
      });
    }
    ...
    if (data.recipients && Array.isArray(data.recipients)) {
      data.recipients.forEach((email, index) => {
        try {
          ...
          throw new Error(
            `[CorrespondenceApiService.update] Invalid email at recipients[${index}]: ${email}`,
          );
        }
      });
    }
  ...
  async updateStatus(
    ...
    if (!status) {
      throw new Error(
        "[CorrespondenceApiService.updateStatus] status is required",
      );
    }
