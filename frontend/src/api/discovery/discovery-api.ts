...
export class DiscoveryApiService {
  ...
  /**
   * Get discovery requests (facade for DiscoveryRequestsApiService)
   */
  async getRequests(caseId?: string): Promise<DiscoveryRequest[]> {
    const api = new DiscoveryRequestsApiService();
    return api.getAll(caseId ? { caseId } : undefined) as unknown as Promise<
      DiscoveryRequest[]
    >;
  }

  /**
   * Get legal holds (facade for LegalHoldsApiService)
   */
  async getLegalHolds(caseId?: string): Promise<LegalHold[]> {
    const api = new LegalHoldsApiService();
    return api.getAll(caseId ? { caseId } : undefined);
  }

  /**
   * Get privilege log (facade for PrivilegeLogApiService)
   */
  async getPrivilegeLog(caseId?: string): Promise<PrivilegeLogEntry[]> {
    const api = new PrivilegeLogApiService();
    return api.getAll(caseId ? { caseId } : undefined);
  }

  /**
   * Get productions (facade for ProductionsApiService)
   */
  async getProductions(caseId?: string): Promise<Production[]> {
    const api = new ProductionsApiService();
    return api.getAll(caseId ? { caseId } : undefined);
  }
