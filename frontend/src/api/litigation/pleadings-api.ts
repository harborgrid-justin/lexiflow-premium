...
export class PleadingsApiService {
  ...
  async getAll(filters?: PleadingFilters): Promise<Pleading[]> {
    try {
      ...
      const response = await apiClient.get<
        Pleading[] | PaginatedResponse<Pleading>
      >(url);
  ...
  async add(data: Partial<Pleading>): Promise<Pleading> {
  ...
  async getTemplates(): Promise<unknown[]> {
    try {
      return await apiClient.get<unknown[]>(`${this.baseUrl}/templates`);
    } catch (error) {
      console.error("[PleadingsApiService.getTemplates] Error:", error);
      return [];
    }
  }
