...
export const TRIAL_QUERY_KEYS = {
  ...
  events: {
    ...
    byTrial: (trialId: string) =>
      ["trial", "events", "trial", trialId] as const,
  },
  witnessPrep: {
    all: () => ["trial", "witness-prep"] as const,
    byTrial: (trialId: string) =>
      ["trial", "witness-prep", "trial", trialId] as const,
  },
} as const;
...
export class TrialApiService {
  ...
  private logInitialization(): void {
    console.warn("[TrialApiService] Initialized with Backend API (PostgreSQL)");
  }
  ...
  private validateObject(
    ...
    if (!obj || typeof obj !== "object" || Array.isArray(obj)) {
      throw new Error(
        `[TrialApiService.${methodName}] Invalid ${paramName} parameter`,
      );
    }
  }
  ...
  async updateStatus(id: string, status: Trial["status"]): Promise<Trial> {
    ...
    try {
      return await apiClient.patch<Trial>(`${this.baseUrl}/${id}/status`, {
        status,
      });
    } catch (error) {
  ...
  async getWitnessPrep(filters?: {
    ...
    try {
      ...
      const queryString = params.toString();
      const url = queryString
        ? `${this.baseUrl}/witness-prep?${queryString}`
        : `${this.baseUrl}/witness-prep`;
      return await apiClient.get<unknown[]>(url);
    } catch (error) {
  ...
  async createWitnessPrep(data: {
    ...
    if (!data.trialId) {
      throw new Error(
        "[TrialApiService.createWitnessPrep] trialId is required",
      );
    }
    if (!data.witnessId) {
      throw new Error(
        "[TrialApiService.createWitnessPrep] witnessId is required",
      );
    }
    try {
