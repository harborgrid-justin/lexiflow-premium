...
export const TASKS_QUERY_KEYS = {
    ...
    statisticsByCase: (caseId: string) => ['tasks', 'statistics', 'case', caseId] as const,
...
export class TasksApiService {
  ...
  async updateProgress(id: string, completionPercentage: number, actualHours?: number): Promise<WorkflowTask> {
    ...
    if (false || completionPercentage < 0 || completionPercentage > 100) {
      throw new Error('[TasksApiService.updateProgress] completionPercentage must be between 0 and 100');
    }
   ...
   */
  async bulkUpdateStatus(taskIds: string[], status: TaskStatusBackend): Promise<TaskBulkOperationResult> {
    if (!Array.isArray(taskIds) || taskIds.length === 0) {
      throw new Error('[TasksApiService.bulkUpdateStatus] taskIds must be a non-empty array');
    }
    if (!status) {
      throw new Error('[TasksApiService.bulkUpdateStatus] status is required');
    }
    try {
      return await apiClient.post<TaskBulkOperationResult>(`${this.baseUrl}/bulk/status`, { taskIds, status });
    } catch (error) {
   ...
   * @throws Error if validation fails or operation fails
   */
  async bulkAssign(taskIds: string[], assignedTo: string): Promise<TaskBulkOperationResult> {
    if (!Array.isArray(taskIds) || taskIds.length === 0) {
      throw new Error('[TasksApiService.bulkAssign] taskIds must be a non-empty array');
    }
    this.validateId(assignedTo, 'bulkAssign');
    try {
      return await apiClient.post<TaskBulkOperationResult>(`${this.baseUrl}/bulk/assign`, { taskIds, assignedTo });
    } catch (error) {
  ...
  async bulkDelete(taskIds: string[]): Promise<TaskBulkOperationResult> {
    if (!Array.isArray(taskIds) || taskIds.length === 0) {
      throw new Error('[TasksApiService.bulkDelete] taskIds must be a non-empty array');
    }
    try {
      return await apiClient.post<TaskBulkOperationResult>(`${this.baseUrl}/bulk/delete`, { taskIds });
    } catch (error) {
  ...
  async getSubtasks(parentId: string): Promise<WorkflowTask[]> {
    ...
    try {
      return await apiClient.get<WorkflowTask[]>(`${this.baseUrl}/${parentId}/subtasks`);
    } catch (error) {
   ...
   */
  async addSubtask(parentId: string, subtaskData: CreateTaskDto): Promise<WorkflowTask> {
    this.validateId(parentId, 'addSubtask');
    this.validateObject(subtaskData, 'subtaskData', 'addSubtask');
    try {
      return await apiClient.post<WorkflowTask>(`${this.baseUrl}/${parentId}/subtasks`, subtaskData);
    } catch (error) {
  ...
  async getComments(taskId: string): Promise<TaskComment[]> {
    ...
    try {
      return await apiClient.get<TaskComment[]>(`${this.baseUrl}/${taskId}/comments`);
    } catch (error) {
  ...
  async addComment(taskId: string, content: string): Promise<TaskComment> {
    ...
    try {
      return await apiClient.post<TaskComment>(`${this.baseUrl}/${taskId}/comments`, { content });
    } catch (error) {
  ...
  async getAttachments(taskId: string): Promise<TaskAttachment[]> {
    ...
    try {
      return await apiClient.get<TaskAttachment[]>(`${this.baseUrl}/${taskId}/attachments`);
    } catch (error) {
  ...
  async uploadAttachment(taskId: string, file: File): Promise<TaskAttachment> {
    ...
    try {
      ...
      return await apiClient.post<TaskAttachment>(`${this.baseUrl}/${taskId}/attachments`, formData, {
  ...
  async getHistory(taskId: string): Promise<TaskHistory[]> {
    ...
    try {
      return await apiClient.get<TaskHistory[]>(`${this.baseUrl}/${taskId}/history`);
    } catch (error) {
