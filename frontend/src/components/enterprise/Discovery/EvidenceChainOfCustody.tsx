  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // If evidenceId provided, find it
        if (evidenceId) {
          const found = items.find((i: any) => i.id === evidenceId);
          if (found) setSelectedEvidence(found as unknown as EvidenceItem);
        }

        // TODO: Fetch transfers and logs for selected evidence
        // const transfers = await DataService.discovery.getTransfers(selectedEvidence.id);
      } catch (err) {
