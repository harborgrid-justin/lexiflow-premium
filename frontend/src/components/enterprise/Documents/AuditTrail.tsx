 ...
 *
 * Enterprise document audit trail with:
...
export function AuditTrail({
  ...
  const uniqueUsers = Array.from(new Set(events.map(e => e.userName))).sort();
  ...
  const filteredEvents = events.filter(event => {
    ...
    // User filter
    if (filterUser !== 'all' && event.userName !== filterUser) {
      return false;
    }
  ...
  const getEventIcon = (eventType: AuditEventType): { icon: JSX.Element; color: string } => {
    switch (eventType) {
      case 'created':
        return {
          icon: (
        ...
        {showFilters && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
            ...
            <select
              ...
              onChange={(e) => setFilterEventType(e.target.value as AuditEventType | 'all')}
              className="px-3 py-2 text-sm border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100"
            >
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    {/* Event Icon */}
                    <div className={`flex-shrink-0 flex items-center justify-center w-10 h-10 rounded-full ${color}`}>
                              ...
                              <dl className="grid grid-cols-2 gap-2 text-xs">
                                {event.metadata.userAgent && (
                                  <>
                                    ...
                                    <dd className="text-gray-700 dark:text-gray-300 font-mono truncate">
                                      {event.metadata.userAgent}
                                    </dd>
                                  </>
                                )}
