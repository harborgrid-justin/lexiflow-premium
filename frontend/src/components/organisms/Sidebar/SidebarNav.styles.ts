...
export const getNavItemButton = (theme: Theme, isActive: boolean) => cn(
  ...
  isActive
    ? cn(theme.action.primary.surface, theme.action.primary.text)
...
export const getActiveIndicator = (theme: Theme) => cn(
  "absolute left-0 top-1/2 -translate-y-1/2 w-1 h-5 rounded-r-full",
  theme.action.primary.bg
);

export const getSubmenuContainer = (theme: Theme) => cn(
  "ml-4 mt-1 space-y-0.5 border-l-2 pl-2",
