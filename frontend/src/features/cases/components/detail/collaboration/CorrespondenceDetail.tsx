...
export const CorrespondenceDetail: React.FC<CorrespondenceDetailProps> = ({ item, type, onClose, onReply }) => {
    ...
    const { mutate: archiveItem } = useMutation(
        async (id: string) => {
            const correspondence = (await DataService.correspondence) as typeof CorrespondenceService;
    ...
    const handleSaveToCase = async () => {
        ...
        try {
            const documents = (await DataService.documents) as DocumentRepository;
    ...
    const handleLinkDocket = async () => {
        ...
        try {
            const docket = (await DataService.docket) as DocketRepository;
    ...
    const handleUploadProof = async () => {
        ...
        const evidence = (await DataService.evidence) as EvidenceRepository;
        try {
            await evidence.add(proof);
            notify.success('Proof of Service uploaded.');
        } catch {
