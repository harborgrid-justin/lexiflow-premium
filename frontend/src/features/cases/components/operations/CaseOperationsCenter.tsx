    ...
    <div className={cn('h-full flex flex-col', isDark ? 'bg-slate-900' : 'bg-slate-50')}>
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <StatCard title="Active Tasks" value={stats.active.toString()} isDark={isDark} />
          <StatCard title="Due Today" value={stats.dueToday.toString()} isDark={isDark} />
          <StatCard title="In Progress" value={stats.inProgress.toString()} isDark={isDark} />
          <StatCard title="Completed" value={stats.completed.toString()} isDark={isDark} />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-2 p-6">
            ...
            {tasksLoading ? (
              ...
              <div className={cn('text-center py-8 text-sm', isDark ? 'text-slate-400' : 'text-slate-600')}>
                No tasks found
              </div>
            ) : (
          ...
          <div className="space-y-6">
            <Card className="p-6">
              ...
              <div className="space-y-3">
                {(Array.isArray(teamMembers) ? teamMembers : (teamMembers as any)?.data || []).slice(0, 5).map((member: User) => (
...
const StatCard: React.FC<{ title: string; value: string; isDark: boolean }> = ({ title, value, isDark }) => (
  <Card className="p-4">
    <div className={cn('text-xs font-medium mb-1', isDark ? 'text-slate-400' : 'text-slate-600')}>
      {title}
    </div>
      ...
      <div className="flex items-start gap-3 flex-1">
        ...
        <div className="flex-1">
          <div className={cn('font-medium', isDark ? 'text-slate-100' : 'text-slate-900')}>
            {task.templateId || 'Workflow Instance'}
          </div>
          <div className={cn('text-sm mt-1', isDark ? 'text-slate-400' : 'text-slate-600')}>
            Status: {task.status}
          </div>
          <div className="flex items-center gap-2 mt-2">
            {task.status === 'running' && <Badge variant="info">In Progress</Badge>}
...
export default CaseOperationsCenter;
