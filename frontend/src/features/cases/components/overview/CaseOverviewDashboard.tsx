 ...
 * - Guideline 21: Pure render logic with complex data aggregation (useMemo)
...
export const CaseOverviewDashboard: React.FC<{ caseId?: string }> = ({ caseId }) => {
  ...
  const { data: recentActivity } = useQuery(
    ...
    async (): Promise<RecentActivity[]> => {
      ...
      const recentMatters = [...allMatters]
        .filter(m => m.updatedAt || m.createdAt)
        .sort((a, b) => {
      ...
      recentMatters.forEach(matter => {
        const timestampStr = matter.updatedAt || matter.createdAt;
        if (!timestampStr) return;
        const updated = new Date(timestampStr);
        if (now.getTime() - updated.getTime() < 24 * 60 * 60 * 1000) {
          activities.push({
            ...
            matterId: matter.id,
            matterTitle: matter.title,
            description: `Status: ${matter.status || 'Unknown'}`,
  ...
  if (mattersError) {
    return <ErrorState message="Failed to load matters data" />;
  }
...
const KPICard: React.FC<KPICardProps> = ({ icon: Icon, title, value, change, trend, isDark }) => (
  <Card className="p-6">
    <div className="flex items-start justify-between">
      <div className="flex-1">
        ...
        <div className={cn('text-3xl font-bold', isDark ? 'text-slate-100' : 'text-slate-900')}>
      ...
      <div className={cn('p-3 rounded-lg', isDark ? 'bg-slate-700' : 'bg-slate-100')}>
        <Icon className={cn('w-6 h-6', isDark ? 'text-blue-400' : 'text-blue-600')} />
      </div>
    </div>
  </Card>
);

const MetricCard: React.FC<{ label: string; value: string; isDark: boolean }> = ({ label, value, isDark }) => (
  <Card className="p-4">
    <div className={cn('text-xs font-medium mb-1', isDark ? 'text-slate-400' : 'text-slate-600')}>
      {label}
    </div>
    <div className={cn('text-xl font-bold', isDark ? 'text-slate-100' : 'text-slate-900')}>
      {value}
    </div>
  </Card>
);
