...
export const getSearchIcon = (theme: Theme) =>
  cn("h-4 w-4 flex-shrink-0", theme.text.tertiary);
...
export const getSuggestionIcon = (theme: Theme, isSelected: boolean) =>
  cn("flex-shrink-0", isSelected ? theme.primary.text : theme.text.tertiary);

export const suggestionContentContainer = "flex-1 min-w-0";

export const getSuggestionText = (theme: Theme, isSelected: boolean) =>
  cn("text-sm truncate", isSelected ? theme.primary.text : theme.text.primary);

export const getSuggestionMetadata = (theme: Theme, isSelected: boolean) =>
  cn("text-xs mt-0.5", isSelected ? theme.primary.text : theme.text.tertiary);
