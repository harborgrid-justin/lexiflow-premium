...
//
// DATA FLOW:
// SERVER (auth endpoint) → LOADER → AUTH PROVIDER → CONSUMER COMPONENTS
//
// ================================================================================
...
function logAuditEvent(event: AuthEvent): void {
  try {
    ...
    if (event.type === 'login' || event.type === 'logout' || event.type === 'access_denied') {
...
interface AuthProviderProps {
  children: ReactNode;
  /**
   * Initial auth state from loader (ENTERPRISE STANDARD)
...
export function AuthProvider({ children, initialAuth }: AuthProviderProps) {
  ...
  const [user, setUser] = useState<AuthUser | null>(() => {
    // Hydrate from loader if available
    if (initialAuth && typeof initialAuth === 'object' && 'user' in initialAuth) {
      return (initialAuth as { user: AuthUser }).user;
    }
    return null;
  });
  ...
  const [session, setSession] = useState<SessionInfo | null>(() => {
    if (initialAuth && typeof initialAuth === 'object' && 'session' in initialAuth) {
      return (initialAuth as { session: SessionInfo }).session;
    }
    return null;
  });
