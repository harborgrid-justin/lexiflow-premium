...
const HOSTS = {
  ...
  DEV: "dev-api.lexiflow.com",
  STAGING: "staging-api.lexiflow.com",
  PROD: "api.lexiflow.com",
};

const URLS = {
  api: (host: string) => `http://${host}/api`,
};
...
export function createConfigFromEnv(): DataSourceConfig {
  const env = (import.meta.env.MODE ||
    "development") as DataSourceEnvironmentConfig["environment"];
  const apiBaseUrl = import.meta.env.VITE_API_BASE_URL || URLS.api(HOSTS.LOCAL);
  const apiVersion = (import.meta.env.VITE_API_VERSION || "v2") as
    | "v1"
    | "v2"
    | "v3";
