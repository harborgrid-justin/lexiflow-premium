    ...
    if (!processes || processes.length === 0) {
        return (
            <EmptyState
                ...
                icon={Search}
                action={<Button variant="primary" icon={Play} onClick={onCreateProcess}>Create Process</Button>}
            />
        );
    }
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <div className={cn("p-2 rounded border shadow-sm transition-all", theme.surface.default, theme.border.default, `group-hover:shadow-md`)}>
                                {getProcessIcon() as React.ReactNode}
                            </div>
                            <div>
            ...
            <div
                ...
                className={cn("border-2 border-dashed rounded-lg flex flex-col items-center justify-center p-8 transition-all cursor-pointer min-h-[200px]", theme.border.default, theme.text.tertiary, `hover:${theme.primary.border}`, `hover:${theme.primary.text}`, `hover:${theme.surface.highlight}`)}
            >
