 ...
 * @description Individual RFP display with sections and progress
 */
...
export const RFPCard: React.FC<RFPCardProps> = React.memo(({ rfp }) => {
  ...
  const getStatusColor = (status: RFP['status']) => {
    ...
    if (status === 'Lost' || status === 'Declined') return "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400";
  ...
  const getSectionStatusColor = (status: string) => {
    if (status === 'Complete') return "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400";
    if (status === 'Review') return "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400";
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {rfp.sections.map((section, idx) => (
              <div key={idx} className="flex justify-between items-center">
                ...
                <span className={cn("px-2 py-0.5 rounded text-xs", getSectionStatusColor(section.status))}>
                  {section.status}
                </span>
              </div>
            ))}
