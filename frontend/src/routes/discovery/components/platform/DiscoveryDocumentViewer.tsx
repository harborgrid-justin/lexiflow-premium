...
export const DiscoveryDocumentViewer: React.FC<DiscoveryDocumentViewerProps> = ({ docId, onBack }) => {
    ...
    const isLoading = isLoadingMetadata || isLoadingContent;

    if (isLoading) {
        return (
            <div className="flex items-center justify-center h-full">
                <LazyLoader message="Loading document..." />
            </div>
        );
    }
    ...
    if (!metadata) {
        return (
            <div className="flex flex-col items-center justify-center h-full">
                <p className={theme.text.secondary}>Document not found.</p>
    ...
    const doc = {
        ...
        type: metadata.type,
        date: metadata.uploadDate ? new Date(metadata.uploadDate).toLocaleDateString() : 'Unknown Date'
    };
