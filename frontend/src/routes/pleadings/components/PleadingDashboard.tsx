...
export const PleadingDashboard: React.FC<PleadingDashboardProps> = ({ onCreate, onEdit, caseId }) => {
    ...
    const { pleadings, templates, createPleading } = usePleadingData();

    const { data: cases = [] } = useQuery<Case[]>(
        ['cases', 'all'],
        () => [] // TODO: use useCases hook when available
    );

    const { mutate: createPleadingMutation } = { mutate: createPleading }; // Wrapper for compatibility
    {
        onSuccess: (newDoc) => {
            createModal.close();
