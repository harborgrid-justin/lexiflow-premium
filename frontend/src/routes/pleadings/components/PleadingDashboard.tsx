...
export const PleadingDashboard: React.FC<PleadingDashboardProps> = ({ onCreate, onEdit, caseId }) => {
    ...
    const { pleadings, templates, savePleading } = usePleadingData();
    ...
    const handleCreateSubmit = async () => {
        ...
        const sections: PleadingSection[] = (template?.defaultSections ?? []).map((s, idx) => ({
            id: `sec-${Date.now()}-${idx}`,
            type: (s.type as PleadingSectionType) ?? 'Paragraph',
            content: s.content ?? '',
            order: idx
        }));
