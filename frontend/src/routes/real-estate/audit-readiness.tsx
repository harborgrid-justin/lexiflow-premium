...
interface LoaderData {
  ...
  stats: { total: number; ready: number; needsAttention: number };
} // ============================================================================
type RealEstateAuditApi = {
  getAuditItems: () => Promise<RealEstateAuditItem[]>;
  updateAuditItem: (
    auditId: string,
    payload: Partial<RealEstateAuditItem>,
  ) => Promise<void>;
}; // ============================================================================
// Meta Tags
// ============================================================================
export function meta() {
  return createMeta({
    ...
    description:
      "Track audit preparation and compliance status for real estate assets including documentation verification.",
  });
} // ============================================================================
// Loader
// ============================================================================
export async function loader(
  ...
  try {
    const realEstate = (DataService as unknown as {
      realEstate: RealEstateAuditApi;
    }).realEstate;
    const auditItems = await realEstate.getAuditItems();
    return {
      data: auditItems,
      stats: {
        total: auditItems.length,
        ready: auditItems.filter(
          (a: RealEstateAuditItem) =>
            a.status === "Ready" || a.status === "Completed",
        ).length,
        needsAttention: auditItems.filter(
          (a: RealEstateAuditItem) => a.status === "Needs Attention",
        ).length,
      },
    };
  } catch (error: unknown) {
    console.error("Failed to fetch audit readiness data:", error);
    return { data: [], stats: { total: 0, ready: 0, needsAttention: 0 } };
  }
} // ============================================================================
// Action
// ============================================================================
export async function action({ request }: ActionFunctionArgs) {
  ...
  switch (intent) {
    case "update": {
      ...
      await realEstate.updateAuditItem(id, {
        status,
  ...
  return (
    <div className="p-8">
      ...
      <nav className="mb-4 text-sm text-[var(--color-textMuted)] dark:text-[var(--color-textMuted)]">
        ...
        <span className="text-[var(--color-text)]">Audit Readiness</span>{" "}
      </nav>{" "}
      {/* Header */}{" "}
      <div className="mb-8 flex items-center justify-between">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--color-textMuted)] dark:text-[var(--color-textMuted)]">
            {" "}
            Prepare for audits and maintain compliance documentation for real
            estate assets.{" "}
          </p>{" "}
        </div>{" "}
        <button
        ...
        >
          {" "}
          <Plus className="h-4 w-4" /> New Audit Item{" "}
        </button>{" "}
      </div>{" "}
      {/* Stats Cards */}{" "}
        ...
        <div className="rounded-lg border border-[var(--color-borderLight)] bg-[var(--color-surface)] p-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="rounded-lg bg-slate-100 p-2">
              {" "}
              <ClipboardCheck className="h-5 w-5 text-[var(--color-textMuted)]" />{" "}
            </div>{" "}
            <div>
              {" "}
      ...
      {data.length === 0 ? (
        <div className="rounded-lg border border-[var(--color-borderLight)] bg-[var(--color-surfaceRaised)] p-12 text-center /50">
          {" "}
          <FileSearch className="mx-auto h-12 w-12 text-[var(--color-textMuted)]" />{" "}
          ...
          >
            {" "}
            <Plus className="h-4 w-4" /> Create First Audit Item{" "}
          </button>{" "}
        </div>
      ) : (
              ...
              {data.map((audit) => (
                ...
                >
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-right text-sm">
                    ...
                    <button
                      onClick={() =>
                        navigate(`/real_estate/audit_readiness/${audit.id}`)
                      }
