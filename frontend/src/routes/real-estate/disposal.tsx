...
type RealEstateDisposalApi = {
  getDisposals: () => Promise<RealEstateDisposal[]>;
  createDisposal: (payload: Partial<RealEstateDisposal>) => Promise<void>;
  updateDisposal: (id: string, payload: Partial<RealEstateDisposal>) => Promise<void>;
...
export function meta() {
  return createMeta({
    ...
    description:
      "Manage disposal and divestiture of surplus real estate assets including valuation and transfer processes.",
  });
} // ============================================================================
// Loader
// ============================================================================
export async function loader(
  ...
  try {
    const realEstate = (DataService as unknown as {
      realEstate: RealEstateDisposalApi;
    }).realEstate;
    const disposals = await realEstate.getDisposals();
    const pendingDisposals = disposals.filter(
      (d: RealEstateDisposal) =>
    ...
    return {
      ...
      stats: {
        ...
        completed: disposals.filter(
          (d: RealEstateDisposal) => d.status === "Completed",
        ).length,
      },
    };
  } catch (error) {
    console.error("Failed to fetch disposal data:", error);
    return { data: [], stats: { total: 0, pending: 0, completed: 0 } };
  }
} // ============================================================================
// Action
// ============================================================================
export async function action({ request }: ActionFunctionArgs) {
  ...
  switch (intent) {
    case "create": {
      ...
      const disposalType = formData.get(
        "disposalType",
      ) as RealEstateDisposal["disposalType"];
      ...
      await realEstate.createDisposal({
        propertyId,
    ...
    case "update": {
      ...
      await realEstate.updateDisposal(id, updates);
    ...
    case "delete": {
      ...
      await realEstate.deleteDisposal(id);
...
export default function DisposalRoute() {
  ...
  const getStatusColor = (status: DisposalStatus): string => {
    switch (status) {
      ...
      case "In Progress":
        return "text-[var(--color-primary)] bg-blue-50 dark:bg-blue-900/20";
      case "Completed":
  ...
  return (
    <div className="p-8">
      ...
      <nav className="mb-4 text-sm text-[var(--color-textMuted)] dark:text-[var(--color-textMuted)]">
        ...
        <span className="text-[var(--color-text)]">Disposal</span>{" "}
      </nav>{" "}
      {/* Header */}{" "}
      <div className="mb-8 flex items-center justify-between">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--color-textMuted)] dark:text-[var(--color-textMuted)]">
            {" "}
            Manage the disposal and divestiture of surplus real estate
            assets.{" "}
          </p>{" "}
        </div>{" "}
        <button
        ...
        <div className="rounded-lg border border-[var(--color-borderLight)] bg-[var(--color-surface)] p-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="rounded-lg bg-slate-100 p-2">
              {" "}
              <Package className="h-5 w-5 text-[var(--color-textMuted)]" />{" "}
            </div>{" "}
            <div>
              {" "}
      ...
      {data.length === 0 ? (
        <div className="rounded-lg border border-[var(--color-borderLight)] bg-[var(--color-surfaceRaised)] p-12 text-center /50">
          {" "}
          <Package className="mx-auto h-12 w-12 text-[var(--color-textMuted)]" />{" "}
          ...
          >
            {" "}
            <Plus className="h-4 w-4" /> Create First Disposal{" "}
          </button>{" "}
        </div>
      ) : (
              ...
              {data.map((disposal) => (
                ...
                >
                  ...
                  <td className="whitespace-nowrap px-6 py-4">
                    {" "}
                    <span
                      className={`inline-flex rounded-full px-2 py-1 text-xs font-semibold ${getStatusColor(disposal.status)}`}
                    >
                      {" "}
                      {disposal.status}{" "}
                    </span>{" "}
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-right text-sm">
                    ...
                    <button
                      onClick={() =>
                        navigate(`/real_estate/disposal/${disposal.id}`)
                      }
