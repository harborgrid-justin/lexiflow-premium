...
export function meta() {
  return createMeta({
    ...
    description:
      "View comprehensive overview of real estate portfolio including property counts, acreage, and value assessments.",
  });
} // ============================================================================
// Loader
// ============================================================================
export async function loader(
  ...
  try {
    const realEstate = (DataService as unknown as {
      realEstate: RealEstatePortfolioApi;
    }).realEstate;
    const stats = await realEstate.getPortfolioStats();
    return { stats };
  } catch (error: unknown) {
...
export default function PortfolioSummaryRoute() {
  ...
  const quickLinks = [
    {
      label: "Inventory",
      to: "/real_estate/inventory",
      description: "View all properties",
    },
    {
      label: "Acquisitions",
      to: "/real_estate/acquisition",
      description: "Pending acquisitions",
    },
    {
      label: "Disposals",
      to: "/real_estate/disposal",
      description: "Property disposals",
    },
    {
      label: "Utilization",
      to: "/real_estate/utilization",
  ...
  return (
    <div className="p-8">
      ...
      <nav className="mb-4 text-sm text-[var(--color-textMuted)] dark:text-[var(--color-textMuted)]">
        ...
        <span className="text-[var(--color-text)]">Portfolio Summary</span>{" "}
      </nav>{" "}
      {/* Header */}{" "}
      <div className="mb-8">
        {" "}
        ...
        <p className="mt-1 text-sm text-[var(--color-textMuted)] dark:text-[var(--color-textMuted)]">
          {" "}
          Comprehensive overview of your real estate portfolio with key metrics
          and analytics.{" "}
        </p>{" "}
      </div>{" "}
      {/* Key Metrics Grid */}{" "}
        ...
        <div className="rounded-lg border border-[var(--color-borderLight)] bg-[var(--color-surface)] p-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-[var(--color-textMuted)] dark:text-[var(--color-textMuted)]">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-3xl font-bold text-teal-600 dark:text-teal-400">
                {formatNumber(stats.totalAcreage)}
              </p>{" "}
              <p className="text-xs text-[var(--color-textMuted)] dark:text-[var(--color-textMuted)]">
                acres
              </p>{" "}
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-3xl font-bold text-rose-600 dark:text-rose-400">
                {stats.pendingAcquisitions + stats.pendingDisposals}
              </p>{" "}
              <p className="text-xs text-[var(--color-textMuted)] dark:text-[var(--color-textMuted)]">
                {stats.pendingAcquisitions} acq / {stats.pendingDisposals} disp
              </p>{" "}
            </div>{" "}
          </div>{" "}
        </div>{" "}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          {quickLinks.map((link) => (
            <Link
              key={link.to}
              to={link.to}
              className="rounded-lg border border-[var(--color-borderLight)] bg-[var(--color-surface)] p-4 hover:border-blue-300 hover:shadow-md transition-all dark:hover:border-blue-600"
            >
              {" "}
      ...
      {stats.activeEncroachments > 0 && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 dark:border-amber-700 dark:bg-amber-900/20">
          ...
          <p className="mt-1 text-sm text-amber-600 dark:text-amber-400">
            {" "}
            There are {stats.activeEncroachments} active encroachment
            {stats.activeEncroachments !== 1 ? "s" : ""} that require
            attention.{" "}
          </p>{" "}
          <Link
