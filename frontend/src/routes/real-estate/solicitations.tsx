...
type RealEstateSolicitationApi = {
  getSolicitations: () => Promise<RealEstateSolicitation[]>;
  createSolicitation: (
    payload: Partial<RealEstateSolicitation>,
  ) => Promise<void>;
}; // ============================================================================
// Meta Tags
// ============================================================================
export function meta() {
  return createMeta({
    ...
    description:
      "Manage real estate solicitations including RFPs, bid invitations, and procurement activities.",
  });
} // ============================================================================
// Loader
// ============================================================================
export async function loader(
  ...
  try {
    const realEstate = (DataService as unknown as {
      realEstate: RealEstateSolicitationApi;
    }).realEstate;
    const solicitations = await realEstate.getSolicitations();
    return {
      data: solicitations,
      stats: {
        total: solicitations.length,
        open: solicitations.filter(
          (s: RealEstateSolicitation) => s.status === "Open",
        ).length,
        awarded: solicitations.filter(
          (s: RealEstateSolicitation) => s.status === "Awarded",
        ).length,
      },
    };
  } catch (error: unknown) {
    console.error("Failed to fetch solicitations data:", error);
    return { data: [], stats: { total: 0, open: 0, awarded: 0 } };
  }
} // ============================================================================
// Action
// ============================================================================
export async function action({ request }: ActionFunctionArgs) {
  ...
  switch (intent) {
    case "create": {
      ...
      const solicitationType = formData.get(
        "solicitationType",
      ) as RealEstateSolicitation["solicitationType"];
      ...
      await realEstate.createSolicitation({
        title,
  ...
  return (
    <div className="p-8">
      ...
      <nav className="mb-4 text-sm text-[var(--color-textMuted)] dark:text-[var(--color-textMuted)]">
        ...
        <span className="text-[var(--color-text)]">Solicitations</span>{" "}
      </nav>{" "}
      {/* Header */}{" "}
      <div className="mb-8 flex items-center justify-between">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--color-textMuted)] dark:text-[var(--color-textMuted)]">
            {" "}
            Track and manage real estate procurement solicitations and bid
            processes.{" "}
          </p>{" "}
        </div>{" "}
        <button
        ...
        >
          {" "}
          <Plus className="h-4 w-4" /> New Solicitation{" "}
        </button>{" "}
      </div>{" "}
      {/* Stats Cards */}{" "}
      ...
      {data.length === 0 ? (
        <div className="rounded-lg border border-[var(--color-borderLight)] bg-[var(--color-surfaceRaised)] p-12 text-center /50">
          ...
          >
            {" "}
            <Plus className="h-4 w-4" /> Create First Solicitation{" "}
          </button>{" "}
        </div>
      ) : (
              ...
              {data.map((solicitation) => (
                ...
                >
                  ...
                  <td className="whitespace-nowrap px-6 py-4">
                    {" "}
                    <span
                      className={`inline-flex rounded-full px-2 py-1 text-xs font-semibold ${getStatusColor(solicitation.status)}`}
                    >
                      {" "}
                      {solicitation.status}{" "}
                    </span>{" "}
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-right text-sm">
                    ...
                    <button
                      onClick={() =>
                        navigate(
                          `/real_estate/solicitations/${solicitation.id}`,
                        )
                      }
