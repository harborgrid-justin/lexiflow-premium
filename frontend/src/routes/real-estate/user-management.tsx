...
type RealEstateUserApi = {
  getModuleUsers: () => Promise<ModuleUser[]>;
...
export function meta() {
  return createMeta({
    ...
    description:
      "Manage user access, roles, and permissions for the real estate module.",
  });
} // ============================================================================
// Loader
// ============================================================================
export async function loader(
  ...
  try {
    const realEstate = (DataService as unknown as {
      realEstate: RealEstateUserApi;
    }).realEstate;
    const users = await realEstate.getModuleUsers();
    return {
      data: users,
      stats: {
        total: users.length,
    ...
    return { data: [], stats: { total: 0, admins: 0, active: 0 } };
  }
} // ============================================================================
// Action
// ============================================================================
export async function action({ request }: ActionFunctionArgs) {
  ...
  switch (intent) {
    case "update": {
      ...
      const permissions =
        (formData.get("permissions") as string)?.split(",").filter(Boolean) ||
        [];
      if (!userId) {
        return { success: false, error: "User ID is required" };
      }
      await realEstate.updateUserPermissions(userId, permissions);
  ...
  return (
    <div className="p-8">
      ...
      <nav className="mb-4 text-sm text-[var(--color-textMuted)] dark:text-[var(--color-textMuted)]">
        ...
        <span className="text-[var(--color-text)]">User Management</span>{" "}
      </nav>{" "}
      {/* Header */}{" "}
      <div className="mb-8 flex items-center justify-between">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--color-textMuted)] dark:text-[var(--color-textMuted)]">
            {" "}
            Configure user access, roles, and permissions for real estate
            operations.{" "}
          </p>{" "}
        </div>{" "}
        <button
        ...
        <div className="rounded-lg border border-[var(--color-borderLight)] bg-[var(--color-surface)] p-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="rounded-lg bg-slate-100 p-2">
              {" "}
              <Users className="h-5 w-5 text-[var(--color-textMuted)]" />{" "}
            </div>{" "}
            <div>
              {" "}
      ...
      {data.length === 0 ? (
        <div className="rounded-lg border border-[var(--color-borderLight)] bg-[var(--color-surfaceRaised)] p-12 text-center /50">
          {" "}
          <Users className="mx-auto h-12 w-12 text-[var(--color-textMuted)]" />{" "}
              ...
              {data.map((user) => (
                ...
                >
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-right text-sm">
                    ...
                    <button
                      onClick={() =>
                        navigate(`/real_estate/user_management/${user.id}`)
                      }
