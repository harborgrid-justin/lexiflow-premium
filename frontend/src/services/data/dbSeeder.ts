...
import { MOCK_DEPOSITIONS, MOCK_ESI_SOURCES, MOCK_PRODUCTIONS, MOCK_INTERVIEWS } from '@/api/types/discoveryExtended';
...
import { MOCK_JUDGE_STATS, MOCK_OUTCOME_DATA } from '@/api/types/analyticsStats';
...
import { MOCK_WIKI_ARTICLES, MOCK_PRECEDENTS, MOCK_QA_ITEMS } from '@/api/types/mockKnowledge';
...
import { MOCK_LEADS, MOCK_CRM_ANALYTICS } from '@/api/types/crm';
import { MOCK_REALIZATION_DATA, MOCK_OPERATING_SUMMARY } from '@/api/types/billingStats';
import { MOCK_DISCOVERY_FUNNEL, MOCK_DISCOVERY_CUSTODIANS } from '@/api/types/discoveryCharts';
...
export const Seeder = {
  async seed(db: DatabaseManager) {
      ...
      await Promise.all([
          ...
          batchPut(STORES.JUDGE_MOTION_STATS, MOCK_JUDGE_STATS.map((stat) => ({...stat, id: `jms-${i}`}))),
          batchPut(STORES.OUTCOME_PREDICTIONS, MOCK_OUTCOME_DATA.map((d) => ({...d, id: `op-${i}`}))),
          batchPut(STORES.OKRS, MOCK_OKRS),
          batchPut(STORES.MALWARE_SIGNATURES, MALWARE_SIGNATURES.map((sig) => ({id: `sig-${i}`, signature: sig}))),
