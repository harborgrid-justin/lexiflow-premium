...
export const EVIDENCE_QUERY_KEYS = {
    ...
    byCustodian: (custodian: string) => ['evidence', 'custodian', custodian] as const,
...
export class EvidenceRepository extends Repository<EvidenceItem> {
     ...
     */
    override async getAll(): Promise<EvidenceItem[]> {
        if (this.useBackend) {
            try {
     ...
     */
    override async update(id: string, updates: Partial<EvidenceItem>): Promise<EvidenceItem> {
        ...
        try {
            ...
            if (updates.admissibility && updates.admissibility !== existing.admissibility) {
                try {
                    ...
                    console.warn('[EvidenceRepository] Failed to publish integration event', eventError);
                }
            }
