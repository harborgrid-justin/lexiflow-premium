...
export const ORGANIZATION_QUERY_KEYS = {
  ...
  byJurisdiction: (jurisdiction: string) =>
    ["organizations", "jurisdiction", jurisdiction] as const,
} as const;
...
export class OrganizationRepository extends Repository<Organization> {
  ...
  private validateId(id: string, methodName: string): void {
    if (!id || id.trim() === "") {
      throw new Error(
        `[OrganizationRepository.${methodName}] Invalid id parameter`
      );
    }
  }
  ...
  override async add(item: Organization): Promise<Organization> {
    if (!item || typeof item !== "object") {
      throw new ValidationError(
        "[OrganizationRepository.add] Invalid organization data"
      );
    }
    try {
      return (await this.orgsApi.create(
  ...
  override async update(
    ...
    try {
      return (await this.orgsApi.update(
        id,
  ...
  async search(searchTerm: string): Promise<Organization[]> {
    ...
    try {
      return (await this.orgsApi.search(
        searchTerm
      )) as unknown as Organization[];
    } catch (error) {
      console.error("[OrganizationRepository] Backend API error", error);
  ...
  async getByType(type: string): Promise<Organization[]> {
    ...
    try {
      return (await this.orgsApi.getByType(
  ...
  async getByJurisdiction(jurisdiction: string): Promise<Organization[]> {
    ...
    try {
      return (await this.orgsApi.getByJurisdiction(
        jurisdiction
      )) as unknown as Organization[];
    } catch (error) {
      console.error("[OrganizationRepository] Backend API error", error);
