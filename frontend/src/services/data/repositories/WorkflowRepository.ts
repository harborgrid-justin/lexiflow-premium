...
export const WorkflowRepository = {
  getProcesses: async () => {
    try {
      ...
      console.warn(
        "[WorkflowRepository] Error fetching processes from IndexedDB, using mock data:",
        error
      );
  ...
  getTemplates: async () => {
    try {
      ...
      console.warn(
        "[WorkflowRepository] Error fetching templates from IndexedDB, using mock data:",
        error
      );
  ...
  getTasks: async () => {
    try {
      ...
      console.warn(
        "[WorkflowRepository] Error fetching tasks from IndexedDB:",
        error
      );
      return [];
    }
  ...
  deploy: async (templateId: string, context?: { caseId?: string }) => {
    try {
      ...
      const template =
        (await db.get<WorkflowTemplateData>(STORES.TEMPLATES, templateId)) ||
  ...
  getProcessDetails: async (id: string) => {
    try {
      ...
      console.warn(
        "[WorkflowRepository] Error fetching process details:",
        error
      );
      return {
        id,
  ...
  getEngineDetails: async (id: string, type: "case" | "process") => {
    try {
      ...
      const tasks = await db.getByIndex<WorkflowTask>(
        STORES.TASKS,
        "caseId",
        id
      );
      ...
      return {
        ...
        automationRate: "0%", // Placeholder until automation flags are available
      };
    } catch (error) {
      console.warn(
