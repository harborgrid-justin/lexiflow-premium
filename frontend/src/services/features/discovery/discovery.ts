...
export const DiscoveryService = {
  ...
  getDepositions: async (caseId?: string) => {
    try {
      const depositions = await api.depositions.getAll(
  ...
  addDeposition: async (dep: Deposition) => {
    try {
      return await api.depositions.create(
  ...
  updateDepositionStatus: async (id: string, status: string) => {
    try {
      return await api.depositions.update(id, { status } as unknown as Record<
        string,
        unknown
      >);
    } catch {
  ...
  getESISources: async (caseId?: string) => {
    try {
      const sources = await api.esiSources.getAll(
        caseId ? { caseId } : undefined
      );
      return sources;
    } catch {
  ...
  addESISource: async (source: ESISource) => {
    try {
      return await api.esiSources.create(
  ...
  updateESISourceStatus: async (id: string, status: string) => {
    try {
      return await api.esiSources.update(id, { status } as unknown as Record<
        string,
        unknown
      >);
    } catch {
  ...
  createProduction: async (prod: ProductionSet) => {
    try {
      return await api.productions.create(
  ...
  getInterviews: async (caseId?: string) => {
    try {
      const interviews = await api.custodianInterviews.getAll(
        caseId ? { caseId } : undefined
      );
      return interviews;
    } catch {
  ...
  updateInterview: async (id: string, updates: Partial<CustodianInterview>) => {
    try {
      return await api.custodianInterviews.update(
        id,
  ...
  createInterview: async (interview: CustodianInterview) => {
    try {
      return await api.custodianInterviews.create(
  ...
  getRequests: async (caseId?: string) => {
    try {
      const requests = await api.discoveryRequests.getAll(
        caseId ? { caseId } : undefined
      );
      return requests;
    } catch {
  ...
  addRequest: async (req: DiscoveryRequest) => {
    try {
      return await api.discoveryRequests.create(
  ...
  updateRequestStatus: async (id: string, status: string) => {
    try {
      return await api.discoveryRequests.update(id, {
        status,
  ...
  getExaminations: async (caseId?: string) => {
    try {
      const exams = await api.examinations.getAll(
        caseId ? { caseId } : undefined
      );
      return exams;
    } catch {
