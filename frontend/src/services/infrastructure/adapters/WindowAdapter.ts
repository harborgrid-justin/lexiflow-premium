...
export class BrowserWindowAdapter implements IWindowAdapter {
  ...
  setInterval(callback: () => void, ms: number): TimerHandle {
    if (!this.isAvailable) {
      throw new EnvironmentError('setInterval not available in non-browser environment');
    }
  ...
  setTimeout(callback: () => void, ms: number): TimerHandle {
    if (!this.isAvailable) {
      throw new EnvironmentError('setTimeout not available in non-browser environment');
    }
  ...
  addEventListener(event: string, listener: EventListener): void {
    if (this.isAvailable) {
      window.addEventListener(event, listener as unknown as globalThis.EventListener);
    }
  }

  removeEventListener(event: string, listener: EventListener): void {
    if (this.isAvailable) {
      window.removeEventListener(event, listener as unknown as globalThis.EventListener);
    }
  }
  ...
  requestAnimationFrame(callback: () => void): number {
    if (!this.isAvailable) {
      throw new EnvironmentError('requestAnimationFrame not available in non-browser environment');
    }
...
export class TestWindowAdapter implements IWindowAdapter {
  private intervals = new Map<TimerHandle, { callback: () => void; ms: number }>();
  private timeouts = new Map<TimerHandle, { callback: () => void; ms: number }>();
...
export class SSRWindowAdapter implements IWindowAdapter {
  ...
  requestAnimationFrame(_callback: () => void): number {
    throw new EnvironmentError('requestAnimationFrame not available in SSR environment');
  }
 ...
 */
export const defaultWindowAdapter: IWindowAdapter = 
