...
> {
  ...
  async handle(
    payload: SystemEventPayloads[typeof SystemEventType.DOCKET_INGESTED]
    ...
    if (
      entry.type === "Hearing" ||
      (entry.title || "").toLowerCase().includes("hearing")
    ) {
  ...
  private async createResponseDeadline(
    entry: SystemEventPayloads[typeof SystemEventType.DOCKET_INGESTED]["entry"]
  ) {
    const deadlineDate = new Date(
      new Date(entry.date || entry.entryDate || entry.dateFiled).getTime() +
        14 * 24 * 60 * 60 * 1000
    );
  ...
  private async createHearingEvent(
    entry: SystemEventPayloads[typeof SystemEventType.DOCKET_INGESTED]["entry"]
  ) {
