...
- `login()`: Proxies credentials to backend via `authGateway`
- `logout()`: Clears local state (backend clears cookie)
- `loadIdentity()`: Fetches `/me` using session cookie
- **No storage**: `localStorage` is NOT used for tokens

### 5. Centralized Routing

**Location**: `app/routing/paths.ts`

- Single source of truth for all route paths via `PATHS` constant.
- Used by `Sidebar`, `RouterProvider`, and internal links.
