...
import type { Case, Matter } from "./case";
import type { EthicalWall, Risk } from "./compliance-risk";
import type { LegalDocument } from "./documents";
import type { EvidenceItem } from "./evidence";
import type { Invoice, TimeEntry } from "./financial";
import type { Citation } from "./legal-research";
import type { LegalEntity, ServiceJob, StaffMember } from "./misc";
...
export interface SystemEventPayloads {
  ...
  // Opp #7
  [SystemEventType.CITATION_SAVED]: {
    citation: Citation;
    queryContext: string;
  };
  ...
  [SystemEventType.SECURITY_THREAT_DETECTED]: {
    threatCount: number;
    threats: unknown[];
    scannedAt: string;
  };
  ...
  [SystemEventType.CLOUD_SYNC_STARTED]: {
    connectionId: string;
