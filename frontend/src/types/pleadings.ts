...
export interface Pleading extends Omit<BaseEntity, "createdBy" | "updatedBy"> {
  ...
  type:
    ...
    | "opposition"
    | "petition"
    | "response"; // Backend: enum PleadingType
...
export interface Clause extends Omit<BaseEntity, "createdBy" | "updatedBy"> {
  ...
  category:
    ...
    | "discovery"
    | "custom"
    | (string & {}); // Backend: enum ClauseCategory
  tags?: string[]; // Backend: simple-array
...
export interface JointPlan extends BaseEntity {
  caseId: CaseId;
  title: string;
  lastUpdated: string;
  status: string;
...
export interface StipulationRequest extends BaseEntity {
  title: string;
