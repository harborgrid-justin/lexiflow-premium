...
import {
  type DocketEntry,
  type DocketEntryStructuredData,
 ...
 */
export function isValidStructuredData(
  data: unknown,
): data is DocketEntryStructuredData {
  if (!data || typeof data !== "object") return false;

  const record = data as Record<string, unknown>;

  // Required fields
  if (typeof record["actionType"] !== "string" || !record["actionType"].trim())
    return false;
  if (typeof record["actionVerb"] !== "string" || !record["actionVerb"].trim())
    return false;

  // Optional fields - must be strings if present
  if (
    record["documentTitle"] !== undefined &&
  ...
  if (record["filer"] !== undefined && typeof record["filer"] !== "string")
    return false;
  return !(
    record["additionalText"] !== undefined &&
    typeof record["additionalText"] !== "string"
  );
}
...
export function isValidDocketEntryType(type: string): type is DocketEntryType {
  const validTypes: DocketEntryType[] = [
    "Filing",
    "Order",
    "Notice",
    "Minute Entry",
 ...
 */
export function isCompleteDocketEntry(
  entry: Partial<DocketEntry>,
): entry is DocketEntry {
  return !!(
    entry.id &&
    true &&
    entry.caseId &&
 ...
 */
export function sanitizeDocketEntry(
  entry: Partial<DocketEntry>,
): Partial<DocketEntry> {
  ...
  if (sanitized.structuredData) {
    ...
    if (data.documentTitle)
      data.documentTitle = sanitizeString(data.documentTitle);
    if (data.filer) data.filer = sanitizeString(data.filer);
    if (data.additionalText)
      data.additionalText = sanitizeString(data.additionalText);
  }
