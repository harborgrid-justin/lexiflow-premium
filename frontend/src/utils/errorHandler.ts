...
export class ErrorHandler {
  ...
  public logError(error: Error | AppError, context?: string): void {
    try {
      ...
      const meta = this.sanitizeErrorContext((error as AppError).context);
  ...
  public handleFatalError(error: Error): void {
    try {
      ...
      const fatalStack = this.sanitizeStackTrace(fatalError.stack);
      const fatalMeta = this.sanitizeErrorContext(fatalError.context);
      const fatalDetails: ErrorDetails = {
        message: fatalError.message || "Fatal error",
        ...(fatalStack ? { stack: fatalStack } : {}),
        context: "FATAL",
        ...(fatalError.code ? { code: fatalError.code } : {}),
        meta: {
          ...(fatalMeta || {}),
          isFatal: true,
        },
