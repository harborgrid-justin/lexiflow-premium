...
export const NexusPhysics = {
  ...
  simulate: (
    ...
    for (let i = 0; i < count; i++) {
      ...
      buffer[idx + O_VX] = buffer[idx + O_VX]! + dxCenter * CENTER_PULL * alpha;
      buffer[idx + O_VY] = buffer[idx + O_VY]! + dyCenter * CENTER_PULL * alpha;
    }
      ...
      for (let nx = gx - 1; nx <= gx + 1; nx++) {
        for (let ny = gy - 1; ny <= gy + 1; ny++) {
          ...
          for (let k = 0; k < len; k++) {
            ...
            buffer[idxI + O_VX] = buffer[idxI + O_VX]! + fx;
            buffer[idxI + O_VY] = buffer[idxI + O_VY]! + fy;
          }
        }
      }
    }
    ...
    for (let i = 0; i < linkCount; i++) {
      ...
      buffer[idxS + O_VX] = buffer[idxS + O_VX]! + fx;
      buffer[idxS + O_VY] = buffer[idxS + O_VY]! + fy;
      buffer[idxT + O_VX] = buffer[idxT + O_VX]! - fx;
      buffer[idxT + O_VY] = buffer[idxT + O_VY]! - fy;
    }

    // 4. Integration
    for (let i = 0; i < count; i++) {
      const idx = i * NODE_STRIDE;
      buffer[idx + O_VX] = buffer[idx + O_VX]! * DAMPING;
      buffer[idx + O_VY] = buffer[idx + O_VY]! * DAMPING;
      buffer[idx + O_X] = buffer[idx + O_X]! + buffer[idx + O_VX]!;
      buffer[idx + O_Y] = buffer[idx + O_Y]! + buffer[idx + O_VY]!;

      // Bounds
      const r = buffer[idx + O_RAD]!;
      if (buffer[idx + O_X]! < r) {
        buffer[idx + O_X] = r;
        buffer[idx + O_VX] = buffer[idx + O_VX]! * -0.5;
      }
      if (buffer[idx + O_X]! > width - r) {
        buffer[idx + O_X] = width - r;
        buffer[idx + O_VX] = buffer[idx + O_VX]! * -0.5;
      }
      if (buffer[idx + O_Y]! < r) {
        buffer[idx + O_Y] = r;
        buffer[idx + O_VY] = buffer[idx + O_VY]! * -0.5;
      }
      if (buffer[idx + O_Y]! > height - r) {
        buffer[idx + O_Y] = height - r;
        buffer[idx + O_VY] = buffer[idx + O_VY]! * -0.5;
      }
    }
