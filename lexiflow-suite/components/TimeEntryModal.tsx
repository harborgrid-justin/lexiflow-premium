...
export const TimeEntryModal: React.FC<TimeEntryModalProps> = ({ isOpen, onClose, caseId, onSave }) => {
  ...
  return (
    <Modal 
      ...
      size="sm"
    >
      <div className={`p-6 space-y-5 transition-opacity duration-200 ${isPending ? 'opacity-60' : 'opacity-100'}`}>
        <Input 
          label="Matter / Case" 
        ...
        <div className="pt-2">
          <Button variant="primary" className="w-full" onClick={handleSave} isLoading={isPending}>
            Save Time Entry
          </Button>
        </div>
      </div>
