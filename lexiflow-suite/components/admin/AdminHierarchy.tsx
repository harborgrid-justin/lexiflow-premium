...
import { HIERARCHY_USERS, MOCK_GROUPS, MOCK_ORGS } from '../../data/mockHierarchy.ts';
...
export const AdminHierarchy: React.FC = () => {
  ...
  return (
    <div style={{ backgroundColor: 'var(--color-background)', borderColor: 'var(--color-border)' }} className="flex flex-col h-full border rounded-lg overflow-hidden animate-fade-in">
      <div style={{ backgroundColor: 'var(--color-surface)', borderColor: 'var(--color-border)' }} className="p-4 border-b flex justify-between items-center shadow-sm z-10 shrink-0">
        <div>
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        <div style={{ backgroundColor: 'var(--color-surface)', borderColor: 'var(--color-border)' }} className="w-full md:w-1/4 md:min-w-[250px] border-b md:border-b-0 md:border-r flex flex-col h-auto md:h-full shrink-0">
          <div style={{ backgroundColor: 'var(--color-surfaceHover)', borderColor: 'var(--color-border)', color: 'var(--color-textMuted)' }} className="p-3 border-b font-bold text-xs uppercase tracking-wide shrink-0">
            Organizations
          </div>
        ...
        <div style={{ backgroundColor: 'var(--color-surfaceHover)' }} className="w-full md:w-1/4 md:min-w-[250px] border-b md:border-b-0 md:border-r border-slate-200 flex flex-col h-auto md:h-full shrink-0">
                  ...
                  >
                    ...
                    <div className="mt-2 flex flex-wrap gap-1">
                      {group.permissions.slice(0, 2).map(p => (
                        <span key={p} style={{ backgroundColor: 'var(--color-surfaceHover)', color: 'var(--color-textMuted)' }} className="text-[9px] px-1.5 py-0.5 rounded">{p.replace('_', ' ')}</span>
                      ))}
                    </div>
                  </div>
                ))}
        ...
        <div style={{ backgroundColor: 'var(--color-surface)' }} className="flex-1 flex flex-col min-w-[300px] h-full overflow-hidden">
          <div className="p-3 bg-slate-50 border-b border-slate-100 flex justify-between items-center shrink-0">
            ...
            <div className="flex gap-2">
              <span style={{ backgroundColor: 'var(--color-surface)', color: 'var(--color-textMuted)', borderColor: 'var(--color-border)' }} className="text-xs border px-2 py-0.5 rounded">{displayedUsers.length}</span>
                ...
                <tbody className="divide-y divide-slate-50">
                  {displayedUsers.map(user => (
                    <tr key={user.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${user.userType === 'Internal'
