  ...
  return (
    <div className={`flex items-center p-3 bg-white border border-slate-200 rounded-lg hover:shadow-sm transition-all group ${className}`}>
      ...
      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        {onPreview && (
          <button onClick={(e) => { e.stopPropagation(); onPreview(); }} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-slate-100 rounded">
            <Eye className="h-4 w-4" />
          </button>
        )}
        <button onClick={(e) => { e.stopPropagation(); onDownload ? onDownload() : null; }} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-slate-100 rounded">
