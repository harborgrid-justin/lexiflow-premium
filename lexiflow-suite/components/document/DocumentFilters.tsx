...
import { Folder, FolderOpen, HardDrive, Clock, Star, Cloud } from 'lucide-react';
...
export const DocumentFilters: React.FC<DocumentFiltersProps> = ({ currentFolder, setCurrentFolder }) => {
  ...
  return (
    <div className="w-64 bg-white rounded-lg border border-slate-200 flex flex-col h-full shrink-0">
        <div className="p-4 border-b border-slate-100">
            ...
            <div className="space-y-1">
                {smartFolders.map(sf => (
                    <button 
                        key={sf.id}
                        className="w-full flex items-center px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-blue-600 rounded-md transition-colors"
                    >
