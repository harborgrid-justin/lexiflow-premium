...
import { Clock, FileText, Paperclip, Send, X } from 'lucide-react';
...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  return (
    <div style={{ backgroundColor: 'var(--color-surface)', borderColor: 'var(--color-border)' }} className="p-4 border-t shrink-0">
      {pendingAttachments.length > 0 && (
        <div className="flex gap-2 mb-2 overflow-x-auto pb-2">
          {pendingAttachments.map((att, i) => (
            <div key={i} style={{ backgroundColor: 'var(--color-surfaceHover)', borderColor: 'var(--color-border)' }} className="flex items-center rounded-full px-3 py-1 text-xs border shrink-0">
      ...
      <div style={{ backgroundColor: 'var(--color-surfaceHover)', borderColor: 'var(--color-border)' }} className="flex items-center gap-2 border rounded-full px-2 py-2 focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500 transition-all shadow-sm">
        <input type="file" ref={fileInputRef} className="hidden" onChange={onFileSelect} />
        <button
          onClick={() => fileInputRef.current?.click()}
          style={{ color: 'var(--color-textMuted)' }}
          className="p-2 hover:text-blue-600 hover:bg-slate-200 rounded-full transition-colors"
          title="Attach File"
        >
