...
import { Attachment, Conversation } from '../../hooks/useSecureMessenger.ts';
...
export const MessengerChatWindow: React.FC<MessengerChatWindowProps> = ({
  ...
  if (!activeConversation) {
    return (
      <div style={{ backgroundColor: 'var(--color-background)', color: 'var(--color-textMuted)' }} className="flex-1 flex flex-col items-center justify-center p-8 h-full">
        <div style={{ backgroundColor: 'var(--color-surfaceHover)' }} className="h-24 w-24 rounded-full flex items-center justify-center mb-6">
  ...
  return (
    <div style={{ backgroundColor: 'var(--color-background)' }} className="flex-1 flex flex-col h-full">
      <ChatHeader
      ...
      <div className={`flex-1 overflow-hidden flex flex-col transition-opacity duration-200 ${isPending ? 'opacity-70' : 'opacity-100'}`}>
        ...
        {pendingAttachments.length > 0 && (
          <div style={{ backgroundColor: 'var(--color-surface)', borderColor: 'var(--color-border)' }} className="px-4 pt-2 border-t flex gap-2 overflow-x-auto shrink-0">
