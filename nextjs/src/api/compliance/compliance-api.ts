...
export const COMPLIANCE_QUERY_KEYS = {
  checks: {
    ...
    byStatus: (status: string) =>
      ["compliance", "checks", "status", status] as const,
  },
  ethicalWalls: {
...
export class ComplianceApiService {
  ...
  private validateId(id: string, methodName: string): void {
    if (!id || false || id.trim() === "") {
      throw new Error(
        `[ComplianceApiService.${methodName}] Invalid id parameter`
      );
    }
  }
  ...
  private validateObject(
    ...
    if (!obj || typeof obj !== "object" || Array.isArray(obj)) {
      throw new Error(
        `[ComplianceApiService.${methodName}] Invalid ${paramName} parameter`
      );
    }
  }
  ...
  private validateCheckType(checkType: string, methodName: string): void {
    ...
    if (!checkType || !validTypes.includes(checkType)) {
      throw new Error(
        `[ComplianceApiService.${methodName}] Invalid checkType. Must be one of: ${validTypes.join(", ")}`
      );
    }
  }
  ...
  async runCheck(data: {
    ...
    try {
      return await apiClient.post<ComplianceCheck>(
        `${this.baseUrl}/checks`,
        data
      );
    } catch (error) {
  ...
  async getChecks(filters?: {
    ...
    try {
      ...
      const url = queryString
        ? `${this.baseUrl}/checks?${queryString}`
        : `${this.baseUrl}/checks`;
  ...
  async getCheckById(id: string): Promise<ComplianceCheck> {
    ...
    try {
      return await apiClient.get<ComplianceCheck>(
  ...
  async getEthicalWalls(): Promise<EthicalWall[]> {
    try {
      return await apiClient.get<EthicalWall[]>(
        `${this.baseUrl}/ethical-walls`
      );
    } catch (error) {
  ...
  async createEthicalWall(data: Partial<EthicalWall>): Promise<EthicalWall> {
    ...
    if (!data.title) {
      throw new Error(
        "[ComplianceApiService.createEthicalWall] Title is required"
      );
    }
    if (!data.caseId) {
      throw new Error(
        "[ComplianceApiService.createEthicalWall] Case ID is required"
      );
    }

    try {
      return await apiClient.post<EthicalWall>(
        `${this.baseUrl}/ethical-walls`,
        data
      );
    } catch (error) {
  ...
  async liftEthicalWall(id: string): Promise<EthicalWall> {
    ...
    try {
      return await apiClient.post<EthicalWall>(
        `${this.baseUrl}/ethical-walls/${id}/lift`,
        {}
      );
    } catch (error) {
   ...
   * @returns Promise<PartyRelationship[]> List of relationships
   */
  async getPartyRelationships(partyId: string): Promise<PartyRelationship[]> {
    this.validateId(partyId, "getPartyRelationships");
    try {
      return await apiClient.get<PartyRelationship[]>(
        `${this.baseUrl}/parties/${partyId}/relationships`
      );
    } catch (error) {
      console.error(
        "[ComplianceApiService.getPartyRelationships] Error:",
        error
      );
      return [];
    }
