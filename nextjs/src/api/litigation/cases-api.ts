...
import type { Case, MatterType } from "@/types";
...
export class CasesApiService {
  ...
  async getAll(filters?: {
    ...
    return casesArray.map((c: unknown) => {
      ...
      return {
        ...
        matterType: (transformed.matterType ||
          transformed.practiceArea ||
          "General") as MatterType,
        filingDate:
          transformed.filingDate || new Date().toISOString().split("T")[0],
      } as Case;
    });
  }
  ...
  async parse(
    ...
    try {
      return await apiClient.post<unknown>("/cases/import/parse", {
        content,
        options,
      });
    } catch (error) {
