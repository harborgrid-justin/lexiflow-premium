...
/**
 * API Route Handler
 * Handles /api/analytics operations
 *
 * GET /api/analytics - Get resource
 * POST /api/analytics - Create resource
 * PUT /api/analytics - Update resource
 * PATCH /api/analytics - Partial update resource
 * DELETE /api/analytics - Delete resource
 *
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] GET /api/analytics - ${request.headers.get('x-forwarded-for') || 'unknown'}`);

    const response = await proxyToBackend(request, "/api/analytics");
...
// Validate required fields
    ...
    // Log request
    console.log(`[API] POST /api/analytics - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
...
// Validate content type
    ...
    // Log request
    console.log(`[API] PUT /api/analytics - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] PUT /api/analytics error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
...
// For PATCH, body can be partial, but should not be empty
    ...
    // Log request
    console.log(`[API] PATCH /api/analytics - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] PATCH /api/analytics error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] DELETE /api/analytics - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] DELETE /api/analytics error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
