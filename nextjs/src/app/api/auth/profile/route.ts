...
/**
 * API Route Handler
 * Handles /api/auth/profile operations
 *
 * GET /api/auth/profile - Get resource
 * PUT /api/auth/profile - Update resource
 * POST /api/auth/profile - Create resource
 * PATCH /api/auth/profile - Partial update resource
 * DELETE /api/auth/profile - Delete resource
 *
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] GET /api/auth/profile - ${request.headers.get('x-forwarded-for') || 'unknown'}`);

    const response = await proxyToBackend(request, "/api/auth/profile");
...
// Validate content type
    ...
    // Log request
    console.log(`[API] PUT /api/auth/profile - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
...
// Validate required fields
    ...
    // Log request
    console.log(`[API] POST /api/auth/profile - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
...
// For PATCH, body can be partial, but should not be empty
    ...
    // Log request
    console.log(`[API] PATCH /api/auth/profile - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] PATCH /api/auth/profile error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] DELETE /api/auth/profile - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] DELETE /api/auth/profile error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
  ...
  return NextResponse.json(
    { error: 'Method Not Allowed', code: 'METHOD_NOT_ALLOWED', message: 'HEAD method not supported on this endpoint' },
    {
      ...
      headers: {
        ...CORS_HEADERS,
        ...SECURITY_HEADERS,
        'Allow': 'GET, PUT, POST, PATCH, DELETE, OPTIONS'
      }
    }
  );
}