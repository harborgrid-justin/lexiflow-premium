...
/**
 * API Route Handler
 * Handles /api/auth/register operations
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] POST /api/auth/register - ${request.headers.get('x-forwarded-for') || 'unknown'}`);

    const response = await proxyToBackend(request, "/api/auth/register");
