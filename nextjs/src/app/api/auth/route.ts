...
/**
 * API Route Handler
 * Handles /api/auth operations
 *
 * GET /api/auth - Get resource
 * POST /api/auth - Create resource
 * PUT /api/auth - Update resource
 * PATCH /api/auth - Partial update resource
 * DELETE /api/auth - Delete resource
 *
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] GET /api/auth - ${request.headers.get('x-forwarded-for') || 'unknown'}`);

    const response = await proxyToBackend(request, "/api/auth");
    ...
    console.error(`[API] GET /api/auth error:`, error);
    return NextResponse.json(
...
// Validate required fields
    ...
    // Log request
    console.log(`[API] POST /api/auth - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
...
// Validate content type
    ...
    // Log request
    console.log(`[API] PUT /api/auth - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] PUT /api/auth error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
...
// For PATCH, body can be partial, but should not be empty
    ...
    // Log request
    console.log(`[API] PATCH /api/auth - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] PATCH /api/auth error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] DELETE /api/auth - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] DELETE /api/auth error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
