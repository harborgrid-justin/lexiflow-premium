...
/**
 * API Route Handler
 * Handles /api/compliance operations
 *
 * GET /api/compliance - Get resource
 * POST /api/compliance - Create resource
 * PUT /api/compliance - Update resource
 * PATCH /api/compliance - Partial update resource
 * DELETE /api/compliance - Delete resource
 *
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] GET /api/compliance - ${request.headers.get('x-forwarded-for') || 'unknown'}`);

    const response = await proxyToBackend(request, "/api/compliance");
...
// Validate required fields
    ...
    // Log request
    console.log(`[API] POST /api/compliance - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] POST /api/compliance error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
...
// Validate content type
    ...
    // Log request
    console.log(`[API] PUT /api/compliance - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
...
// For PATCH, body can be partial, but should not be empty
    ...
    // Log request
    console.log(`[API] PATCH /api/compliance - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] PATCH /api/compliance error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] DELETE /api/compliance - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] DELETE /api/compliance error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
