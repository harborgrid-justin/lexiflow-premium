...
/**
 * API Route Handler
 * Handles /api/documents operations
 *
 * GET /api/documents - Get resource
 * POST /api/documents - Create resource
 * PUT /api/documents - Update resource
 * PATCH /api/documents - Partial update resource
 * DELETE /api/documents - Delete resource
 *
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] GET /api/documents - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
...
// Validate required fields
    ...
    // Log request
    console.log(`[API] POST /api/documents - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
...
// Validate content type
    ...
    // Log request
    console.log(`[API] PUT /api/documents - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] PUT /api/documents error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
...
// For PATCH, body can be partial, but should not be empty
    ...
    // Log request
    console.log(`[API] PATCH /api/documents - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] PATCH /api/documents error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] DELETE /api/documents - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] DELETE /api/documents error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
