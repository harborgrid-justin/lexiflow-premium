...
/**
 * API Route Handler
 * Handles /api/organizations operations
 *
 * GET /api/organizations - Get resource
 * POST /api/organizations - Create resource
 * PUT /api/organizations - Update resource
 * PATCH /api/organizations - Partial update resource
 * DELETE /api/organizations - Delete resource
 *
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] GET /api/organizations - ${request.headers.get('x-forwarded-for') || 'unknown'}`);

    const response = await proxyToBackend(request, "/api/organizations");
...
// Validate required fields
    ...
    // Log request
    console.log(`[API] POST /api/organizations - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
...
// Validate content type
    ...
    // Log request
    console.log(`[API] PUT /api/organizations - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] PUT /api/organizations error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
...
// For PATCH, body can be partial, but should not be empty
    ...
    // Log request
    console.log(`[API] PATCH /api/organizations - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] PATCH /api/organizations error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] DELETE /api/organizations - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] DELETE /api/organizations error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
