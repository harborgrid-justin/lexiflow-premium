...
/**
 * API Route Handler
 * Handles /api/parties operations
 *
 * GET /api/parties - Get resource
 * POST /api/parties - Create resource
 *
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] GET /api/parties - ${request.headers.get('x-forwarded-for') || 'unknown'}`);

    const response = await proxyToBackend(request, "/api/parties");
    ...
    console.error(`[API] GET /api/parties error:`, error);
    return NextResponse.json(
...
// Validate required fields
    ...
    // Log request
    console.log(`[API] POST /api/parties - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] POST /api/parties error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
