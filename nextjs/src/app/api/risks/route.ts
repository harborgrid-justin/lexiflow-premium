...
/**
 * API Route Handler
 * Handles /api/risks operations
 *
 * GET /api/risks - Get resource
 * POST /api/risks - Create resource
 *
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] GET /api/risks - ${request.headers.get('x-forwarded-for') || 'unknown'}`);

    const response = await proxyToBackend(request, "/api/risks");
...
// Validate required fields
    ...
    // Log request
    console.log(`[API] POST /api/risks - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
