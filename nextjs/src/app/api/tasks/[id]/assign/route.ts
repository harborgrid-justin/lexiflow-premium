...
/**
 * API Route Handler
 * Handles /api/tasks/[id]/assign operations
 *
 * POST /api/tasks/[id]/assign - Assign task to a user
 * DELETE /api/tasks/[id]/assign - Unassign task from user
 *
 ...
 *   "assignedBy"?: "user-uuid",  // Optional - who is making the assignment
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Validate required fields - assignedTo is required for assignment
    ...
    // Log request
    console.log(`[API] POST /api/tasks/${id}/assign - ${request.headers.get('x-forwarded-for') || 'unknown'}`);

    const response = await proxyToBackend(request, `/api/tasks/${id}/assign`);
    ...
    console.error(`[API] POST /api/tasks/[id]/assign error:`, error);
    return NextResponse.json(
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Log request
    console.log(`[API] DELETE /api/tasks/${id}/assign - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] DELETE /api/tasks/[id]/assign error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
