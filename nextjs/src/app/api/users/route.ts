...
/**
 * API Route Handler
 * Handles /api/users operations
 *
 * GET /api/users - Get resource
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] GET /api/users - ${request.headers.get('x-forwarded-for') || 'unknown'}`);

    const response = await proxyToBackend(request, "/api/users");
...
// Validate required fields
    ...
    // Log request
    console.log(`[API] POST /api/users - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
