...
/**
 * API Route Handler
 * Handles /api/webhooks operations
 *
 * GET /api/webhooks - Get resource
 * POST /api/webhooks - Create resource
 *
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] GET /api/webhooks - ${request.headers.get('x-forwarded-for') || 'unknown'}`);

    const response = await proxyToBackend(request, "/api/webhooks");
    ...
    console.error(`[API] GET /api/webhooks error:`, error);
    return NextResponse.json(
...
// Validate required fields
    ...
    // Log request
    console.log(`[API] POST /api/webhooks - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] POST /api/webhooks error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
