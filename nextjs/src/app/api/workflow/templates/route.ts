...
/**
 * API Route Handler
 * Handles /api/workflow/templates operations
 *
 * GET /api/workflow/templates - Get resource
 * POST /api/workflow/templates - Create resource
 *
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] GET /api/workflow/templates - ${request.headers.get('x-forwarded-for') || 'unknown'}`);

    const response = await proxyToBackend(request, "/api/workflow/templates");
    ...
    console.error(`[API] GET /api/workflow/templates error:`, error);
    return NextResponse.json(
...
// Validate required fields
    ...
    // Log request
    console.log(`[API] POST /api/workflow/templates - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] POST /api/workflow/templates error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
