...
/**
 * API Route Handler
 * Handles /api/workflows operations
 *
 * GET /api/workflows - Get resource
 * POST /api/workflows - Create resource
 * PUT /api/workflows - Update resource
 * PATCH /api/workflows - Partial update resource
 * DELETE /api/workflows - Delete resource
 *
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] GET /api/workflows - ${request.headers.get('x-forwarded-for') || 'unknown'}`);

    const response = await proxyToBackend(request, "/api/workflows");
...
// Validate required fields
    ...
    // Log request
    console.log(`[API] POST /api/workflows - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
...
// Validate content type
    ...
    // Log request
    console.log(`[API] PUT /api/workflows - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] PUT /api/workflows error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
...
// For PATCH, body can be partial, but should not be empty
    ...
    // Log request
    console.log(`[API] PATCH /api/workflows - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] PATCH /api/workflows error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Log request
    console.log(`[API] DELETE /api/workflows - ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    ...
    console.error(`[API] DELETE /api/workflows error:`, error);
    return NextResponse.json(
      { error: 'Internal Server Error', code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' },
      {
        status: 500,
