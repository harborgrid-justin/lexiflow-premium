...
// Utils & Constants
import { SEARCH_MIN_QUERY_LENGTH } from '@/config/features/search.config';
...
export const NeuralCommandBar = React.memo<NeuralCommandBarProps>(({
    ...
    // Effect discipline: Synchronize search results with deferred query
    // Strict Mode ready: Search is idempotent, cleanup prevents stale results
    useEffect(() => {
        let isMounted = true;

        const performSearch = async () => {
            if (deferredSearch.length >= SEARCH_MIN_QUERY_LENGTH && !isProcessingIntent) {
                const serviceResults = await SearchService.search(deferredSearch);
    ...
    }, [deferredSearch, isProcessingIntent]);
