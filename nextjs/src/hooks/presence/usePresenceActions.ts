    ...
    async (activity: string | undefined) => {
      ...
      try {
        if (activity) {
          ...
          // Use a separate timeout handler to avoid circular dependency
          activityTimeoutRef.current = setTimeout(async () => {
            if (socket && isConnected) {
              setMyActivity(undefined);
  ...
  const setCustomStatus = useCallback(
    async (status: string | undefined) => {
      ...
      try {
        await emit(
          status
            ? "presence:set-custom-status"
            : "presence:clear-custom-status",
    ...
    async (userIds: string[]) => {
      ...
      try {
        ...
        setPresenceMap((prev) => {
          ...
          userIds.forEach((id) => m.delete(id));
          return m;
        });
      } catch (e) {
