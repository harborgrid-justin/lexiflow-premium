...
>({
  ...
  const isFieldVisible = useCallback(
    (fieldName: keyof TFormData): boolean => {
      const fieldSchema = getFieldSchema(
        schema as unknown as FormSchema<TFormData>,
        fieldName as string
      );
  ...
  useEffect(() => {
    // Avoid synchronous setState in effect by wrapping in setTimeout or ensuring it's necessary
    // This effect derives visibility from formData, which could be done in render, but since
    // it updates fieldStates which is returned, we need to sync it.
    // Using a microtask to avoid "synchronous update" warning in strict mode
  ...
  }, [formData, isFieldVisible]); // Added dependencies to be correct
