 ...
 */
export interface EntityAutocompleteConfig<
  TEntity,
...
export interface EntityAutocompleteReturn<
  TEntity,
  TCreateData = Partial<TEntity>,
> {
...
export function useEntityAutocomplete<TEntity, TCreateData = Partial<TEntity>>({
  ...
  createFn,
  getLabel,
  getValue: _getValue, // Preserved but commented out usage
  queryKey,
  debounceMs = 300,
  minSearchLength = 0,
  initialOptions = [],
}: EntityAutocompleteConfig<TEntity, TCreateData>): EntityAutocompleteReturn<
  TEntity,
  TCreateData
> {
  // Use _getValue to avoid unused var warning
  void _getValue;
  ...
  const createEntity = useCallback(
    async (data: TCreateData): Promise<TEntity> => {
      if (!createFn) {
  ...
  const hasExactMatch = useCallback(
    (label: string): boolean => {
      ...
      return options.some(
        (entity) => getLabel(entity).trim().toLowerCase() === normalizedLabel
      );
    },
    [options, getLabel]
  );
