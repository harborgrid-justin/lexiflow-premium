...
export function useVirtualizedDocket<T>({
  ...
  const [itemOffsets, setItemOffsets] = useState<number[]>([0]);

  // Update calculations in effect to avoid ref access during render
  useEffect(() => {
    let height = 0;
    const offsets: number[] = [0];

    for (let i = 0; i < items.length; i++) {
      const itemHeight = measurementsRef.current.get(i) ?? estimatedItemHeight;
      height += itemHeight;
  ...
  const virtualItems = useMemo(() => {
    ...
    for (let i = startIndex; i <= endIndex; i++) {
      ...
      result.push({
        ...
        item: items[i],
        start: itemOffsets[i],
        size: itemOffsets[i + 1] - itemOffsets[i], // Use offset difference instead of getItemHeight
      });
    }
