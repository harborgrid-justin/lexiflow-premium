...
export const getMatters = createQuery<MatterFilterInput | undefined, Matter[]>(
  async (input, context) => {
    ...
    if (params.priority?.length) queryString.set('priority', params.priority.join(','));
    if (params.clientId) queryString.set('clientId', params.clientId);
    if (params.leadAttorneyId) queryString.set('leadAttorneyId', params.leadAttorneyId);
    if (params.practiceArea) queryString.set('practiceArea', params.practiceArea);
    ...
    return apiRequest<Matter[]>(endpoint, {
      method: 'GET',
      next: {
...
export const getMatterById = createQuery<{ id: string }, Matter | null>(
  async (input, context) => {
    ...
    try {
      return await apiRequest<Matter>(`/matters/${id}`, {
        method: 'GET',
        next: {
 ...
 */
export const getMatterByNumber = createQuery<{ matterNumber: string }, Matter | null>(
  async (input, context) => {
 ...
 */
export const getMatterSummary = createQuery<{ matterId: string }, MatterSummary>(
  async (input, context) => {
    const { matterId } = input;

    return apiRequest<MatterSummary>(`/matters/${matterId}/summary`, {
      method: 'GET',
      next: {
 ...
 */
export const getMatterFinancials = createQuery<{ matterId: string }, MatterFinancials>(
  async (input, context) => {
    const { matterId } = input;

    return apiRequest<MatterFinancials>(`/matters/${matterId}/financials`, {
      method: 'GET',
      next: {
        tags: [CacheTags.MATTER(matterId)],
        revalidate: CacheProfiles.FAST,
...
export const getMattersByClient = createQuery<{ clientId: string }, Matter[]>(
  async (input, context) => {
    ...
    return apiRequest<Matter[]>(`/matters?clientId=${clientId}`, {
      method: 'GET',
      next: {
 ...
 */
export const getMattersByAttorney = createQuery<{ attorneyId: string }, Matter[]>(
  async (input, context) => {
    const { attorneyId } = input;

    return apiRequest<Matter[]>(`/matters?leadAttorneyId=${attorneyId}`, {
      method: 'GET',
      next: {
        tags: [CacheTags.MATTERS],
        revalidate: CacheProfiles.DEFAULT,
...
export const getActiveMatters = createQuery<undefined, Matter[]>(
  async (_, context) => {
    return apiRequest<Matter[]>('/matters?status=ACTIVE', {
      method: 'GET',
      next: {
        tags: [CacheTags.MATTERS],
        revalidate: CacheProfiles.DEFAULT,
...
export const getMattersNeedingConflictCheck = createQuery<undefined, Matter[]>(
  async (_, context) => {
    return apiRequest<Matter[]>('/matters?conflictCheckCompleted=false', {
      method: 'GET',
      next: {
        tags: [CacheTags.MATTERS],
        revalidate: CacheProfiles.DEFAULT,
...
export const createMatter = createMutation<CreateMatterInput, Matter>(
  async (input, context) => {
    ...
    const newMatter = await apiRequest<Matter>('/matters', {
      method: 'POST',
...
export const updateMatter = createMutation<UpdateMatterInput, Matter>(
  async (input, context) => {
    ...
    const updatedMatter = await apiRequest<Matter>(`/matters/${id}`, {
      method: 'PATCH',
 ...
 * Delete a matter
 */
export const deleteMatter = createMutation<{ id: string }, { success: boolean }>(
  async (input, context) => {
    const { id } = parseInput(idInputSchema, input);

    await apiRequest<void>(`/matters/${id}`, {
      method: 'DELETE',
    }, context);
...
export const updateMatterStatus = createMutation<
  { id: string; status: 'INTAKE' | 'ACTIVE' | 'PENDING' | 'ON_HOLD' | 'CLOSED' | 'ARCHIVED' },
  Matter
>(
  async (input, context) => {
...
>(
  async (input, context) => {
    ...
    const updatedMatter = await apiRequest<Matter>(`/matters/${matterId}`, {
      method: 'PATCH',
      body: JSON.stringify({
...
>(
  async (input, context) => {
    ...
    const matter = await apiRequest<Matter>(`/matters/${matterId}`, {
      method: 'GET',
    }, context);
...
>(
  async (input, context) => {
    ...
    const linkedCaseIds = (matter.linkedCaseIds ?? []).filter((id) => id !== caseId);
...
>(
  async (input, context) => {
    ...
    invalidateTags([CacheTags.MATTERS, ...ids.map((id) => CacheTags.MATTER(id))]);
...
export async function createMatterFormAction(
  ...
  // String fields
  const stringFields = [
    'matterNumber', 'title', 'description', 'status', 'matterType', 'priority',
    'practiceArea', 'clientId', 'clientName', 'leadAttorneyId', 'leadAttorneyName',
    'originatingAttorneyId', 'originatingAttorneyName', 'jurisdiction', 'venue',
    'billingType', 'openedDate', 'targetCloseDate', 'opposingPartyName',
    'opposingCounselFirm', 'conflictCheckNotes', 'riskLevel', 'riskNotes',
    'internalNotes',
  ];
  ...
  // Numeric fields
  const numericFields = [
    'hourlyRate', 'flatFee', 'contingencyPercentage', 'retainerAmount',
...
export async function updateMatterFormAction(
  ...
  const stringFields = [
    ...
    'practiceArea', 'clientId', 'clientName', 'leadAttorneyId', 'leadAttorneyName',
    'originatingAttorneyId', 'originatingAttorneyName', 'jurisdiction', 'venue',
    'billingType', 'openedDate', 'targetCloseDate', 'closedDate',
    'opposingPartyName', 'opposingCounselFirm', 'conflictCheckNotes',
    'riskLevel', 'riskNotes', 'internalNotes',
  ];
