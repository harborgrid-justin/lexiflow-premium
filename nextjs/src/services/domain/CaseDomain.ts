...
import { Case, CaseId, CasePhase, CaseStatus, Party } from "@/types";
  ...
  async importDocket(caseId: string, data: unknown) {
    if (isBackendApiEnabled()) {
      try {
        ...
        console.error(
          "[CaseRepository.importDocket] Backend API error:",
          error
        );
  ...
  async flag(id: string) {
    if (isBackendApiEnabled()) {
      try {
        ...
        console.error("[CaseRepository.flag] Backend API error:", error);
        throw new OperationError("CaseRepository.flag", "Failed to flag case");
      }
    }
