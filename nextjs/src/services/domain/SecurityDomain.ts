...
export const SecurityService = {
  ...
  getMalwareSignatures: async (): Promise<string[]> => {
    try {
      ...
      console.warn(
        "[SecurityService] Backend API disabled, returning empty malware signatures"
      );
      return [];
    } catch (error) {
  ...
  scanForMalware: async (
    ...
    try {
      ...
      if (!result.clean) {
        console.warn(
          `[SecurityService] Threats detected: ${threatsFound.length}`
        );
        ...
        try {
          ...
          console.warn(
            "[SecurityService] Failed to publish security event",
            eventError
          );
        }
      }
