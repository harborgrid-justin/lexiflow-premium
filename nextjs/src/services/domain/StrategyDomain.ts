...
export const StrategyService = {
  ...
  getById: async (id: string) => {
    if (isBackendApiEnabled()) {
      try {
        return await apiClient.get<Strategy>(`/litigation/strategies/${id}`);
      } catch (error) {
        console.error("[StrategyService.getById] Backend API error:", error);
        return null;
      }
    }
  ...
  analyzeRisks: async (strategyId: string): Promise<Risk[]> => {
    if (isBackendApiEnabled()) {
      try {
        ...
        throw new OperationError("Failed to analyze risks");
      }
    }
    ...
    const existingRisks: Risk[] =
      strategy && Array.isArray((strategy as Record<string, unknown>).risks)
        ? ((strategy as Record<string, unknown>).risks as Risk[])
        : [];
    ...
    const recommendations: Recommendation[] = [
      {
        ...
        description:
          "Focus on email communications between key decision-makers",
        priority: 0.9,
        rationale:
          "Early case assessment suggests document-heavy evidence trail",
      },
      {
        id: `rec-${Date.now()}-2`,
        type: "motion",
        ...
        priority: 0.75,
        rationale:
          "Opposing counsel requests exceed Rule 26 proportionality limits",
      },
      {
        id: `rec-${Date.now()}-3`,
        type: "settlement",
        ...
        description:
          "Consider early settlement conference before costly depositions",
        priority: 0.65,
