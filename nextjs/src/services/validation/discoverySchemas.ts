...
const validateDiscoveryRequest = (
  ...
  if (
    ...
    !discoveryTypes.includes(dataType as DiscoveryType)
  ) {
  ...
  if (
    ...
    !discoveryRequestStatuses.includes(dataStatus as DiscoveryRequestStatus)
  ) {
...
const validatePrivilegeLogEntry = (
  ...
  if (
    !dataBasis ||
    typeof dataBasis !== "string" ||
    !privilegeBasisTypes.includes(dataBasis as PrivilegeBasis)
  ) {
...
const validateLegalHold = (
  ...
  if (
    ...
    !legalHoldStatuses.includes(record.status as LegalHoldStatus)
  ) {
    errors.push({ path: "status", message: "Invalid status" });
  }
...
const validateESISource = (data: unknown): ValidationResult<ESISource> => {
  ...
  if (
    ...
    !validTypes.includes(input.type as (typeof validTypes)[number])
  ) {
  ...
  if (
    ...
    !esiCollectionStatuses.includes(input.status as ESICollectionStatus)
  ) {
    errors.push({ path: "status", message: "Invalid status" });
  }
