...
const validateEvidenceItem = (
  ...
  if (
    ...
    !evidenceTypes.includes(record.type as EvidenceType)
  ) {
  ...
  if (
    ...
    typeof record.admissibility !== "string" ||
    !admissibilityStatuses.includes(record.admissibility as AdmissibilityStatus)
  ) {
    errors.push({
      path: "admissibility",
...
const validateCustodyEvent = (
  ...
  if (
    ...
    !custodyActions.includes(record.action as CustodyAction)
  ) {
