...
let permissionsStore: RolePermission[] = [
    ...
    { id: 'perm-10' as UUID, role: 'Client', resource: 'Financials', access: 'None' },

    // Audit Logs
    { id: 'perm-11' as UUID, role: 'Administrator', resource: 'Audit Logs', access: 'Read' },
    { id: 'perm-12' as UUID, role: 'Partner', resource: 'Audit Logs', access: 'Read' },
    { id: 'perm-13' as UUID, role: 'Associate', resource: 'Audit Logs', access: 'None' },
    { id: 'perm-14' as UUID, role: 'Paralegal', resource: 'Audit Logs', access: 'None' },
    { id: 'perm-15' as UUID, role: 'Client', resource: 'Audit Logs', access: 'None' },

    // Security Settings
    { id: 'perm-16' as UUID, role: 'Administrator', resource: 'Security Settings', access: 'Full' },
    { id: 'perm-17' as UUID, role: 'Partner', resource: 'Security Settings', access: 'Read' },
    { id: 'perm-18' as UUID, role: 'Associate', resource: 'Security Settings', access: 'None' },
    { id: 'perm-19' as UUID, role: 'Paralegal', resource: 'Security Settings', access: 'None' },
    { id: 'perm-20' as UUID, role: 'Client', resource: 'Security Settings', access: 'None' },
];
...
export const AdminService = {
    ...
    addLog: async (entry: Omit<AuditLogEntry, 'id'>): Promise<AuditLogEntry> => { 
        const log = { id: crypto.randomUUID() as UUID, ...entry };
    ...
    updatePermission: async (payload: { role: string, resource: string, level: string }): Promise<RolePermission> => {
        await delay(100);
        const idx = permissionsStore.findIndex(p => p.role === payload.role && p.resource === payload.resource);
        if (idx !== -1) {
            permissionsStore[idx].access = payload.level as PermissionLevel;
            return permissionsStore[idx];
        } else {
            const newPerm: RolePermission = { id: `perm-${Date.now()}` as UUID, role: payload.role, resource: payload.resource, access: payload.level as PermissionLevel };
            permissionsStore.push(newPerm);
            return newPerm;
        }
    },
