...
export interface Case extends BaseEntity { 
  title: string; client: string; clientId?: string; matterType: MatterType; matterSubType?: string; 
  ...
  ownerId?: string; ownerOrgId?: string; team?: CaseTeamMember[]; linkedCaseIds?: string[]; leadCaseId?: string; 
...
export interface Invoice extends BaseEntity { client: string; matter: string; caseId: string; date: string; dueDate: string; amount: number; status: string; items: string[]; currency?: CurrencyCode; taxAmount?: number; taxJurisdiction?: string; }
...
export interface Client extends BaseEntity { name: string; industry: string; status: string; totalBilled: number; matters: string[]; trustSubLedgers?: TrustSubLedger[]; taxId?: string; billingGuidelineId?: string; }

// --- CLUSTER 4: DOCUMENTS & DISCOVERY ---
export interface LegalDocument extends BaseEntity { 
  caseId: string; title: string; type: string; content: string; searchableText?: string; ocrStatus?: OcrStatus; 
  ...
  linkedRules?: string[]; sharedWith?: string[]; isRedacted?: boolean; authorId?: string; formFields?: any[]; 
...
export interface AccessControlList { roleId?: string; userId?: string; permission: 'Read' | 'Write' | 'None'; }
export interface ReviewBatch extends BaseEntity { name: string; caseId: string; assigneeId: string; documentIds: string[]; status: 'Pending' | 'In Progress' | 'Completed'; dueDate?: string; }
export interface RedactionLog extends BaseEntity { documentId: string; page: number; x: number; y: number; width: number; height: number; reason: string; appliedBy: string; }
export interface ProductionVolume extends BaseEntity { caseId: string; name: string; batesStart: string; batesEnd: string; format: 'PDF' | 'Native' | 'TIFF'; documentCount: number; producedDate: string; }
...
export interface Juror extends BaseEntity { caseId: string; name: string; status: 'Panel' | 'Seated' | 'Struck' | 'Alternate'; strikeParty?: 'Plaintiff' | 'Defense'; notes?: string; demographics?: any; }
export interface Witness extends BaseEntity { caseId: string; name: string; type: 'Fact' | 'Expert'; credibilityScore: number; impeachmentRisks?: string[]; prepStatus: number; linkedExhibits?: string[]; }
...
export interface OpeningStatement extends BaseEntity { caseId: string; sections: { title: string; durationMinutes: number; linkedExhibitIds: string[] }[]; }
export interface Fact extends BaseEntity { caseId: string; date: string; description: string; type: 'Undisputed' | 'Disputed' | 'Stipulated'; supportingEvidenceIds: string[]; }

// --- CLUSTER 6: WORKFLOW & AUTOMATION ---
export interface WorkflowTask extends BaseEntity { 
  title: string; status: TaskStatus; assignee: string; assigneeId?: string; startDate?: string; dueDate: string; 
  priority: 'Low' | 'Medium' | 'High' | 'Critical'; description?: string; caseId?: string; projectId?: string; 
  relatedModule?: string; relatedItemId?: string; relatedItemTitle?: string; actionLabel?: string; 
  automatedTrigger?: string; linkedRules?: string[]; dependencies?: string[]; dependencyType?: TaskDependencyType; 
...
export interface ApprovalChain extends BaseEntity { name: string; steps: { role: UserRole; userId?: string; order: number }[]; }
...
export interface TrialExhibit extends BaseEntity { caseId: string; exhibitNumber: string; title: string; dateMarked: string; party: string; status: string; fileType: string; description?: string; witness?: string; uploadedBy?: string; tags?: string[]; admissibilityHistory?: { date: string; status: string; ruling?: string }[]; }
...
export interface LegalEntity extends BaseEntity { name: string; type: EntityType; roles: EntityRole[]; email?: string; phone?: string; website?: string; address?: string; city?: string; state?: string; country?: string; taxId?: string; company?: string; title?: string; barNumber?: string; jurisdiction?: string; status: 'Active' | 'Inactive' | 'Prospect' | 'Blacklisted' | 'Deceased'; riskScore: number; tags: string[]; notes?: string; linkedUserId?: string; avatar?: string; externalIds?: Record<string, string>; aliases?: string[]; }
export interface EntityRelationship extends BaseEntity { sourceId: string; targetId: string; type: 'Employment' | 'Subsidiary' | 'Counsel_For' | 'Sued_By' | 'Witness_For' | 'Family' | 'Conflict' | 'Board_Member'; description?: string; startDate?: string; endDate?: string; active: boolean; weight?: number; }
